...
import { BarChart3, FileSpreadsheet, FileText, ClipboardList } from 'lucide-react';
...
function PreparationsTab({ query, view }: {
  ...
  const { summary, detailed, summaryTable } = query.data;

  return (
    <div className="space-y-4 sm:space-y-6">
      {/* KPI Summary Cards */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        <SummaryCard title="\u0141\u0105czna liczba us\u0142ug" value={`${summary.totalItems}`} color="purple" />
        <SummaryCard title="\u0141\u0105czna warto\u015b\u0107" value={formatCurrency(summary.totalValue)} color="green" />
        <SummaryCard title="Rezerwacje" value={`${summary.totalReservations}`} color="blue" />
        <SummaryCard title="Dni z wydarzeniami" value={`${summary.daysWithEvents}`} color="orange" />
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
        {summary.topCategory && (
          <div className="bg-white dark:bg-neutral-900 rounded-xl border border-neutral-200 dark:border-neutral-700 p-4">
            <p className="text-xs sm:text-sm text-neutral-500 dark:text-neutral-400">Top kategoria</p>
            <p className="text-base sm:text-lg font-semibold text-purple-700 dark:text-purple-400">{summary.topCategory}</p>
          </div>
        )}
        {summary.nearestEvent && (
          <div className="bg-white dark:bg-neutral-900 rounded-xl border border-neutral-200 dark:border-neutral-700 p-4">
            <p className="text-xs sm:text-sm text-neutral-500 dark:text-neutral-400">Najbli\u017csze wydarzenie</p>
            <p className="text-base sm:text-lg font-semibold text-neutral-900 dark:text-neutral-100">{summary.nearestEvent}</p>
          </div>
        )}
      </div>

      {/* DETAILED VIEW */}
      {view === 'detailed' && detailed && detailed.length > 0 && (
        <div className="space-y-4">
          {detailed.map((day) => (
            <div key={day.dateLabel} className="bg-white dark:bg-neutral-900 rounded-xl border border-neutral-200 dark:border-neutral-700 overflow-hidden">
              <div className="px-4 py-3 bg-neutral-800 dark:bg-neutral-800 border-b border-neutral-200 dark:border-neutral-700 flex items-center justify-between">
                ...
                <span className="text-xs text-neutral-300">
                  {day.dayItemCount} us\u0142ug \u00b7 {formatCurrency(day.dayTotal)}
                </span>
              </div>
              {day.categories.map((cat) => (
                <div key={cat.categoryName} className="border-b border-neutral-100 dark:border-neutral-800 last:border-b-0">
                      ...
                      <tbody className="divide-y divide-neutral-100 dark:divide-neutral-800">
                        {cat.items.map((item, idx) => (
                          <tr key={idx} className="hover:bg-neutral-50 dark:hover:bg-neutral-800/50">
                            ...
                            <td className="px-3 sm:px-4 py-2.5 text-right text-green-700 dark:text-green-400 font-semibold whitespace-nowrap">{formatCurrency(item.totalPrice)}</td>
                            <td className="px-3 sm:px-4 py-2.5 text-neutral-600 dark:text-neutral-400 text-xs hidden sm:table-cell">{item.reservationLabel}</td>
      ...
      {/* SUMMARY VIEW */}
      {view === 'summary' && summaryTable && summaryTable.length > 0 && (
        <div className="bg-white dark:bg-neutral-900 rounded-xl border border-neutral-200 dark:border-neutral-700 overflow-hidden">
          <div className="px-4 py-3 border-b border-neutral-200 dark:border-neutral-700">
            <h3 className="text-sm font-semibold text-neutral-900 dark:text-neutral-100">Zestawienie zbiorcze us\u0142ug</h3>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-neutral-50 dark:bg-neutral-800">
              ...
              <tbody className="divide-y divide-neutral-100 dark:divide-neutral-800">
                {summaryTable.map((item, idx) => (
                  <tr key={idx} className="hover:bg-neutral-50 dark:hover:bg-neutral-800/50">
                    ...
                    <td className="px-3 sm:px-4 py-2.5 text-right text-green-700 dark:text-green-400 font-semibold whitespace-nowrap">{formatCurrency(item.totalValue)}</td>
