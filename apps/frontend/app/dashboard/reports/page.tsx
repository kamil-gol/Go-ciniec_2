    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Analityka przychodów i zajętości sal</p>
        </div>
      ...
      <div className="border-b border-gray-200">
        <nav className="flex gap-4" aria-label="Tabs">
          <button onClick={() => setActiveTab('revenue')}
            className={`py-3 px-1 border-b-2 font-medium text-sm transition-colors ${activeTab === 'revenue' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}>
            Przychody
          </button>
          <button onClick={() => setActiveTab('occupancy')}
            className={`py-3 px-1 border-b-2 font-medium text-sm transition-colors ${activeTab === 'occupancy' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}>
            Zajętość sal
          </button>
        </nav>
      </div>
          ...
          <div className="flex gap-2">
            {presets.map((preset) => (
              <button key={preset.label}
                ...
                className={`px-3 py-1.5 text-xs font-medium rounded-full transition-colors ${dateFrom === preset.dateFrom && dateTo === preset.dateTo ? 'bg-blue-100 text-blue-700 ring-1 ring-blue-300' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
                {preset.label}
              </button>
            ))}
          </div>
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <SummaryCard title="Średnia / rezerwację" value={formatCurrency(summary.avgRevenuePerReservation)} color="green" />
        <SummaryCard title="Wzrost vs okres wcześniej" value={formatPercent(summary.growthPercent)} color={summary.growthPercent >= 0 ? 'green' : 'red'} />
        <SummaryCard title="Rezerwacje" value={`${summary.totalReservations} (${summary.completedReservations} zrealizowanych)`} color="purple" />
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="bg-white rounded-xl border border-gray-200 p-4">
          <p className="text-sm text-gray-500">Najlepszy dzień</p>
          <p className="text-lg font-semibold text-gray-900">{summary.maxRevenueDay || 'Brak danych'}</p>
          <p className="text-sm text-green-600 font-medium">{formatCurrency(summary.maxRevenueDayAmount)}</p>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 p-4">
          <p className="text-sm text-gray-500">Oczekujący przychód</p>
          <p className="text-lg font-semibold text-orange-600">{formatCurrency(summary.pendingRevenue)}</p>
          <p className="text-sm text-gray-500">Z {summary.totalReservations - summary.completedReservations} niezrealizowanych rezerwacji</p>
        </div>
      </div>
              ...
              <tbody className="divide-y divide-gray-100">
                {breakdown.map((item) => (
                  <tr key={item.period} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-2.5 text-right text-green-700 font-semibold">{formatCurrency(item.revenue)}</td>
                    <td className="px-4 py-2.5 text-right text-gray-600">{item.count}</td>
                    <td className="px-4 py-2.5 text-right text-gray-600">{formatCurrency(item.avgRevenue)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {byHall.length > 0 && (
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            <div className="px-4 py-3 border-b border-gray-200">
              <h3 className="text-sm font-semibold text-gray-900">Przychody wg sali</h3>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-gray-50">
                ...
                <tbody className="divide-y divide-gray-100">
                  {byHall.map((item) => (
                    <tr key={item.hallId} className="hover:bg-gray-50">
                      <td className="px-4 py-2.5 font-medium text-gray-900">{item.hallName}</td>
                      <td className="px-4 py-2.5 text-right text-green-700 font-semibold">{formatCurrency(item.revenue)}</td>
                      <td className="px-4 py-2.5 text-right text-gray-600">{item.count}</td>
                    </tr>
                  ))}
                ...
                <tbody className="divide-y divide-gray-100">
                  {byEventType.map((item) => (
                    <tr key={item.eventTypeId} className="hover:bg-gray-50">
                      <td className="px-4 py-2.5 font-medium text-gray-900">{item.eventTypeName}</td>
                      <td className="px-4 py-2.5 text-right text-green-700 font-semibold">{formatCurrency(item.revenue)}</td>
                      <td className="px-4 py-2.5 text-right text-gray-600">{item.count}</td>
                    </tr>
                  ))}
...
function OccupancyTab({ query }: { query: ReturnType<typeof useOccupancyReport> }) {
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <SummaryCard title="Średnia zajętość" value={`${summary.avgOccupancy}%`} color="blue" />
        <SummaryCard title="Najlepszy dzień tygodnia" value={dayNamesPL[summary.peakDay] || summary.peakDay} color="green" />
        <SummaryCard title="Najpopularniejsza sala" value={summary.peakHall || 'Brak danych'} color="purple" />
        <SummaryCard title="Łącznie rezerwacji" value={`${summary.totalReservations} / ${summary.totalDaysInPeriod} dni`} color="orange" />
      </div>

      {halls.length > 0 && (
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          <div className="px-4 py-3 border-b border-gray-200">
            <h3 className="text-sm font-semibold text-gray-900">Zajętość wg sali</h3>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-50">
              ...
              <tbody className="divide-y divide-gray-100">
                {halls.map((hall) => (
                  <tr key={hall.hallId} className="hover:bg-gray-50">
                    <td className="px-4 py-2.5 font-medium text-gray-900">{hall.hallName}</td>
                    <td className="px-4 py-2.5 text-right">
                      <span className="inline-flex items-center">
                        <span className="inline-block w-16 h-2 rounded-full bg-gray-200 mr-2">
              ...
              {peakHours.map((item) => {
                ...
                return (
                  <div key={item.hour} className="flex items-center gap-3">
                    <span className="text-sm font-medium text-gray-600 w-12">{String(item.hour).padStart(2, '0')}:00</span>
              ...
              {peakDaysOfWeek.map((item) => {
                ...
                return (
                  <div key={item.dayOfWeekNum} className="flex items-center gap-3">
                    <span className="text-sm font-medium text-gray-600 w-24">{dayNamesPL[item.dayOfWeek] || item.dayOfWeek}</span>
                    <div className="flex-1 h-6 bg-gray-100 rounded-full overflow-hidden">
