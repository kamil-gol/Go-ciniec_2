...
interface Props { templateId: string; pkg?: CateringPackage | null; onClose: () => void; }
...
export function PackageForm({ templateId, pkg, onClose }: Props) {
  ...
  useEffect(() => {
    if (pkg) {
      form.reset({
        ...
        priceType: pkg.priceType, basePrice: pkg.basePrice,
        minGuests: pkg.minGuests ?? '', maxGuests: pkg.maxGuests ?? '',
        badgeText: pkg.badgeText ?? '', isPopular: pkg.isPopular,
        isActive: pkg.isActive, displayOrder: pkg.displayOrder,
      });
    }
  }, [pkg, form]);
  ...
  const onSubmit = (values: FormValues) => {
    const payload = {
      ...
      isPopular: values.isPopular, isActive: values.isActive, displayOrder: values.displayOrder,
    };
    if (isEdit && pkg) {
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 max-h-[70vh] overflow-y-auto pr-1">
        <div className="rounded-lg border p-4 space-y-4">
          <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Dane pakietu</p>
          <FormField control={form.control} name="name" render={({ field }) => (
            <FormItem><FormLabel>Nazwa pakietu *</FormLabel>
              <FormControl><Input placeholder="np. Standard, Premium, Deluxe" {...field} /></FormControl>
              <FormMessage /></FormItem>
          )} />
          <FormField control={form.control} name="shortDescription" render={({ field }) => (
            <FormItem><FormLabel>Krótki opis (na karcie pakietu)</FormLabel>
              <FormControl><Input placeholder="np. Idealny na 50–100 osób" {...field} /></FormControl>
              <FormMessage /></FormItem>
          )} />
          <FormField control={form.control} name="description" render={({ field }) => (
            <FormItem><FormLabel>Pełny opis</FormLabel>
              <FormControl><Textarea placeholder="Szczegółowy opis pakietu..." rows={3} {...field} /></FormControl>
              <FormMessage /></FormItem>
          )} />
        </div>

        <div className="rounded-lg border p-4 space-y-4">
          <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Cennik</p>
          <div className="grid grid-cols-2 gap-4">
            <FormField control={form.control} name="priceType" render={({ field }) => (
              <FormItem><FormLabel>Typ ceny *</FormLabel>
                <Select onValueChange={field.onChange} value={field.value}>
                  <FormControl><SelectTrigger><SelectValue placeholder="Wybierz..." /></SelectTrigger></FormControl>
                  <SelectContent>{priceTypes.map((k) => <SelectItem key={k} value={k}>{CATERING_PRICE_TYPE_LABELS[k]}</SelectItem>)}</SelectContent>
                </Select><FormMessage /></FormItem>
            )} />
            <FormField control={form.control} name="basePrice" render={({ field }) => (
              <FormItem><FormLabel>Cena bazowa (zł) *</FormLabel>
          ...
          <div className="grid grid-cols-2 gap-4">
            <FormField control={form.control} name="minGuests" render={({ field }) => (
              <FormItem><FormLabel>Min. gości</FormLabel>
                <FormControl><Input type="number" min="1" placeholder="–" {...field} value={field.value ?? ''} /></FormControl>
                <FormMessage /></FormItem>
            )} />
            <FormField control={form.control} name="maxGuests" render={({ field }) => (
              <FormItem><FormLabel>Maks. gości</FormLabel>
                <FormControl><Input type="number" min="1" placeholder="–" {...field} value={field.value ?? ''} /></FormControl>
                <FormMessage /></FormItem>
            )} />
          </div>
        ...
        <div className="rounded-lg border p-4 space-y-4">
          <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Ustawienia</p>
          <div className="grid grid-cols-2 gap-4">
            <FormField control={form.control} name="badgeText" render={({ field }) => (
              <FormItem><FormLabel>Tekst badge&apos;a</FormLabel>
                <FormControl><Input placeholder="np. Bestseller" {...field} /></FormControl>
                <FormMessage /></FormItem>
            )} />
            <FormField control={form.control} name="displayOrder" render={({ field }) => (
              <FormItem><FormLabel>Kolejność</FormLabel>
          ...
          <div className="flex items-center gap-6">
            <FormField control={form.control} name="isPopular" render={({ field }) => (
              <FormItem className="flex items-center gap-2">
                ...
                <FormLabel className="!mt-0">Popularny</FormLabel></FormItem>
            )} />
            <FormField control={form.control} name="isActive" render={({ field }) => (
              <FormItem className="flex items-center gap-2">
                <FormControl><Switch checked={field.value} onCheckedChange={field.onChange} /></FormControl>
                <FormLabel className="!mt-0">Aktywny</FormLabel></FormItem>
            )} />
          </div>
        </div>
