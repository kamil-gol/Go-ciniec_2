...
import type { ServiceItem, AssignExtraInput } from '@/types/service-extra.types'
...
export function selectedExtrasToInput(extras: SelectedExtra[]): AssignExtraInput[] {
  return extras.map((e) => ({
...
export function CreateReservationExtrasSection({
  ...
  const toggleItem = useCallback(
    (item: ServiceItem) => {
      if (selectedMap.has(item.id)) {
        ...
        onExtrasChange([...filtered, { serviceItemId: item.id, serviceItem: item, quantity: 1, note: '' }])
      } else {
        const filtered = selectedExtras.filter(
  ...
  const updateQuantity = useCallback(
    (serviceItemId: string, delta: number) => {
      onExtrasChange(
  ...
  const updateNote = useCallback(
    (serviceItemId: string, note: string) => {
      onExtrasChange(
        selectedExtras.map((e) => (e.serviceItemId === serviceItemId ? { ...e, note } : e))
      )
    },
    [selectedExtras, onExtrasChange]
  )
  ...
  const getItemPrice = useCallback(
    (item: ServiceItem, qty: number = 1): string => {
      ...
      >
        ...
        <div className="flex items-center gap-2">
          {selectedExtras.length > 0 && (
            <Badge
              variant="secondary"
              className="bg-rose-100 dark:bg-rose-900/50 text-rose-700 dark:text-rose-300"
            >
            ...
            {isLoading ? (
              ...
              <div className="p-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg flex items-center gap-2">
                ...
                <p className="text-sm text-amber-800 dark:text-amber-200">
                  Brak dostępnych usług dodatkowych. Skonfiguruj katalog w{' '}
                  <span className="font-medium">Ustawienia → Usługi dodatkowe</span>.
                </p>
              </div>
            ) : (
                      ...
                      >
                        <div className="flex items-center gap-2">
                          ...
                          {selectedInCategory.length > 0 && (
                            <Badge
                              variant="secondary"
                              className="text-xs bg-rose-100 dark:bg-rose-900/50 text-rose-600 dark:text-rose-400"
                            >
