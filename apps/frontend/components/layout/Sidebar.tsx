          ...
          {filteredNav.map((item, index) => {
            ...
            return (
              ...
              >
                ...
                {isActive && (
                  <motion.div
                    layoutId={isCollapsed ? 'sidebarActiveDesktop' : 'sidebarActiveMobile'}
...
export default function Sidebar({ user, onLogout, mobileOpen, onMobileClose }: SidebarProps) {
  ...
  const mobileFilteredNav = navigation.filter((item) =>
  ...
  return (
    <>
      ...
      <Sheet open={mobileOpen} onOpenChange={(open) => !open && onMobileClose?.()}>
        <SheetContent side="left" className="w-[280px] p-0 border-r border-neutral-200/80 dark:border-neutral-700/50">
          <div className="flex h-full flex-col">
            ...
            {/* Nav + User — always expanded (no collapse on mobile) */}
            <SidebarNav
              pathname={pathname}
              searchQuery={mobileSearchQuery}
              setSearchQuery={setMobileSearchQuery}
              isCollapsed={false}
              filteredNav={mobileFilteredNav}
            />
            <SidebarUser user={user} onLogout={onLogout} isCollapsed={false} />
          </div>
        </SheetContent>
      </Sheet>
    </>
