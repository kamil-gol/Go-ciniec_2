          ...
          <div key={step.key} className="flex items-center flex-1 min-w-0">
            <div className="flex flex-col items-center gap-1.5 min-w-[64px]">
              <div className={cn(
                ...
                {isCompleted ? <CheckCircle2 className="h-5 w-5" /> : <Icon className={cn('h-5 w-5', isCurrent && step.key === 'IN_PROGRESS' && 'animate-spin')} />}
              </div>
              <span className={cn(
            ...
            {idx < statusSteps.length - 1 && (
              <div className={cn('flex-1 h-0.5 mx-1 rounded-full transition-all min-w-[16px]', isCompleted ? 'bg-success-500' : 'bg-border')} />
            )}
          </div>
        )
      })}
