...
export function CreateReservationForm({
  ...
  const selectedEventType = useMemo(
    () => eventTypesArray.find((t) => t.id === selectedEventTypeId),
    [eventTypesArray, selectedEventTypeId]
  )
  const selectedEventTypeName = selectedEventType?.name || ''
  const standardHours = selectedEventType?.standardHours ?? 6
  const extraHourRate = selectedEventType?.extraHourRate ?? 500
