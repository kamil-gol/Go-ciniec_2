  ...
  return (
    <div className="space-y-6">
      <div className="text-center mb-6">
        ...
        <p className="text-sm text-neutral-500 dark:text-neutral-300 mt-1">Przejrzyj dane przed utworzeniem rezerwacji</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="p-4 rounded-xl border bg-purple-50 dark:bg-purple-900/20 border-purple-200 dark:border-purple-800 cursor-pointer hover:border-purple-400 dark:hover:border-purple-600 transition-colors" onClick={() => goToStep(0)}>
          ...
          {isBirthday && watchAll.birthdayAge && <p className="text-sm text-neutral-600 dark:text-neutral-300">{watchAll.birthdayAge}. urodziny</p>}
          {isAnniversary && watchAll.anniversaryYear && <p className="text-sm text-neutral-600 dark:text-neutral-300">{watchAll.anniversaryYear}. rocznica</p>}
          {isCustom && watchAll.customEventType && <p className="text-sm text-neutral-600 dark:text-neutral-300">{watchAll.customEventType}</p>}
        </div>
        ...
        <div className="p-4 rounded-xl border bg-blue-50 dark:bg-blue-900/20 border-blue-200 dark:border-blue-800 cursor-pointer hover:border-blue-400 dark:hover:border-blue-600 transition-colors" onClick={() => goToStep(1)}>
          ...
          {durationHours > 0 && (
            <p className={`text-xs ${durationHours > standardHours ? 'text-amber-700 dark:text-amber-300 font-medium' : 'text-neutral-500 dark:text-neutral-300'}`}>
        ...
        <div className="p-4 rounded-xl border bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800 cursor-pointer hover:border-green-400 dark:hover:border-green-600 transition-colors" onClick={() => goToStep(2)}>
          ...
          <p className="text-sm text-neutral-600 dark:text-neutral-300">{adults} dor. {children > 0 && `+ ${children} dzieci `}{toddlers > 0 && `+ ${toddlers} mal.`}</p>
        </div>
        ...
        <div className="p-4 rounded-xl border bg-indigo-50 dark:bg-indigo-900/20 border-indigo-200 dark:border-indigo-800 cursor-pointer hover:border-indigo-400 dark:hover:border-indigo-600 transition-colors" onClick={() => goToStep(4)}>
          ...
          ) : <p className="text-neutral-500 dark:text-neutral-300">—</p>}
        </div>
      </div>
      ...
      <div className="space-y-4 pt-4 border-t border-neutral-200 dark:border-neutral-700">
        ...
        <p className="-mt-2 text-xs text-neutral-500 dark:text-neutral-300">Musi być co najmniej 1 dzień przed rozpoczęciem wydarzenia</p>

        <div>
          <div className="flex items-center gap-2 mb-1"><FileText className="w-5 h-5 text-neutral-500 dark:text-neutral-300" /><label className="block text-sm font-medium text-neutral-700 dark:text-neutral-300">Notatki</label></div>
