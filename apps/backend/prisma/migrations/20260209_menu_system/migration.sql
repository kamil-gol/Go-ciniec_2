...
-- Constraints
ALTER TABLE "MenuOption" ADD CONSTRAINT "MenuOption_priceType_check" CHECK ("priceType" IN ('PER_PERSON', 'FLAT', 'FREE'));

ALTER TABLE "MenuPriceHistory" ADD CONSTRAINT "MenuPriceHistory_entityType_check" CHECK ("entityType" IN ('TEMPLATE', 'PACKAGE', 'OPTION'));

-- Comments
COMMENT ON TABLE "MenuTemplate" IS 'Menu templates for event types with validity period';
COMMENT ON TABLE "MenuPackage" IS 'Pricing packages within menu templates (Basic, Standard, Premium)';
COMMENT ON TABLE "MenuOption" IS 'Additional services/items available for packages';
COMMENT ON TABLE "MenuPackageOption" IS 'Junction table linking packages to available options';
COMMENT ON TABLE "ReservationMenuSnapshot" IS 'Immutable snapshot of selected menu for reservation (preserves historical data)';
COMMENT ON TABLE "MenuPriceHistory" IS 'Audit trail for all price changes in menu system';
