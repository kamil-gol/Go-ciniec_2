...
const svc = auditLogService as any;
  ...
  describe('getAuditLogs()', () => {
    it('should pass query filters with defaults', async () => {
      ...
      await auditLogController.getAuditLogs(
        req({ query: { entityType: 'RESERVATION', page: '2', pageSize: '25' } }), response
      );
      expect(svc.getAuditLogs).toHaveBeenCalledWith(
        expect.objectContaining({ entityType: 'RESERVATION', page: 2, pageSize: 25 })
      );
