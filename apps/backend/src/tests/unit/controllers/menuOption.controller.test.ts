...
describe('MenuOptionController', () => {
  ...
  describe('create()', () => {
    it('should forward 401 to next', async () => {
      await menuOptionController.create(
        req({ user: undefined, body: { name: 'O', category: 'DRINK', priceType: 'FLAT' } }), res(), next
      );
    ...
    it('should return 400 on invalid category', async () => {
      const response = res();
      await menuOptionController.create(
        req({ body: { name: 'O', category: 'INVALID', priceType: 'FLAT' } }), response, next
      );
      expect(response.status).toHaveBeenCalledWith(400);
    });
    ...
    it('should return 201 on success', async () => {
      ...
      const response = res();
      await menuOptionController.create(
        req({ body: { name: 'Cola', category: 'DRINK', priceType: 'PER_PERSON' } }), response, next
      );
  ...
  describe('update()', () => {
    it('should forward 401 to next', async () => {
      await menuOptionController.update(
        req({ user: undefined, params: { id: 'o-1' }, body: {} }), res(), next
      );
      expect(next).toHaveBeenCalledWith(expect.objectContaining({ statusCode: 401 }));
    });
    ...
    it('should return 400 on invalid category', async () => {
      const response = res();
      await menuOptionController.update(
        req({ params: { id: 'o-1' }, body: { category: 'BAD' } }), response, next
      );
      expect(response.status).toHaveBeenCalledWith(400);
    });
    ...
    it('should return 404', async () => {
      ...
      const response = res();
      await menuOptionController.update(
        req({ params: { id: 'x' }, body: { name: 'U' } }), response, next
      );
      expect(response.status).toHaveBeenCalledWith(404);
    });
    ...
    it('should return 200 on success', async () => {
      ...
      const response = res();
      await menuOptionController.update(
        req({ params: { id: 'o-1' }, body: { name: 'Updated' } }), response, next
      );
      expect(response.json).toHaveBeenCalledWith(expect.objectContaining({ success: true }));
    });
  });
