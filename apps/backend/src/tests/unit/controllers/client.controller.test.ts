...
const svc = clientService as any;
...
describe('ClientController', () => {
  ...
  describe('createClient()', () => {
    it('should throw 401 when no user', async () => {
      await expect(controller.createClient(
        req({ user: undefined, body: { firstName: 'Jan', lastName: 'K', phone: '+48123' } }), res()
      )).rejects.toMatchObject({ statusCode: 401 });
    });

    it('should throw 400 when required fields missing', async () => {
      await expect(controller.createClient(
        req({ body: { firstName: 'Jan' } }), res()
      )).rejects.toMatchObject({ statusCode: 400 });
    });

    it('should throw 400 on invalid email', async () => {
      await expect(controller.createClient(
        req({ body: { firstName: 'Jan', lastName: 'K', phone: '+48123', email: 'bad' } }), res()
      )).rejects.toMatchObject({ statusCode: 400 });
    });
    ...
    it('should return 201 on valid creation', async () => {
      ...
      await controller.createClient(
        req({ body: { firstName: 'Jan', lastName: 'Kowalski', phone: '+48123456789' } }), response
      );
  ...
  describe('updateClient()', () => {
    it('should throw 401 when no user', async () => {
      await expect(controller.updateClient(
        req({ user: undefined, params: { id: 'c-1' } }), res()
      )).rejects.toMatchObject({ statusCode: 401 });
    });

    it('should throw 400 on invalid email', async () => {
      await expect(controller.updateClient(
        req({ params: { id: 'c-1' }, body: { email: 'invalid' } }), res()
      )).rejects.toMatchObject({ statusCode: 400 });
    });
    ...
    it('should return 200 on success', async () => {
      ...
      await controller.updateClient(
        req({ params: { id: 'c-1' }, body: { firstName: 'Updated' } }), response
      );
      expect(response.status).toHaveBeenCalledWith(200);
    });
  });
  ...
  describe('deleteClient()', () => {
    it('should throw 401 when no user', async () => {
      await expect(controller.deleteClient(
        req({ user: undefined, params: { id: 'c-1' } }), res()
      )).rejects.toMatchObject({ statusCode: 401 });
    });
