...
const svc = discountService as any;
...
describe('DiscountController', () => {
  describe('applyDiscount()', () => {
    it('should throw 401 when no user', async () => {
      await expect(controller.applyDiscount(
        req({ user: undefined, body: { type: 'PERCENTAGE', value: 10, reason: 'VIP' } }), res()
      )).rejects.toMatchObject({ statusCode: 401 });
    });

    it('should throw 400 when fields missing', async () => {
      await expect(controller.applyDiscount(
        req({ body: { type: 'PERCENTAGE' } }), res()
    ...
    it('should return 200 on valid discount', async () => {
      ...
      await controller.applyDiscount(
        req({ body: { type: 'PERCENTAGE', value: 10, reason: 'VIP client' } }), response
      );
  ...
  describe('removeDiscount()', () => {
    ...
    it('should return 200 on success', async () => {
      ...
      expect(response.json).toHaveBeenCalledWith(
        expect.objectContaining({ message: 'Discount removed successfully' })
      );
    });
  });
});
