...
jest.mock('@prisma/client', () => {
  return {
    ...
    Decimal: jest.fn().mockImplementation((val: string) => ({
      toString: () => val,
    })),
  };
});
...
const makeDecimal = (val: number) => ({ toString: () => val.toString() });
...
describe('calculatePrice — PER_ADULT option', () => {
  it('should calculate PER_ADULT price (basePrice * adults * qty)', async () => {
    ...
    await calculatePrice(req as Request, res as Response);
...
describe('getAvailablePackages — with date filter', () => {
  it('should pass date OR clause when date query param provided', async () => {
    ...
    await getAvailablePackages(req as Request, res as Response);

    expect(mockMenuPackageFindMany).toHaveBeenCalledWith(
      expect.objectContaining({
...
describe('calculateOptionPrice — PER_GUEST_TYPE', () => {
  it('should calculate PER_GUEST_TYPE for single option endpoint', async () => {
    ...
    await calculateOptionPrice(req as Request, res as Response);
