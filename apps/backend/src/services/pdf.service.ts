...
export interface CateringOrderPDFData {
  ...
  createdAt: Date;
    ...
    return new Promise((resolve, reject) => {
      const doc = new PDFDocument({
        size: 'A4',
        margins: { top: 0, bottom: 30, left: 40, right: 40 },
        info: { Title: `Zamówienie ${data.orderNumber}`, Author: this.restaurantData.name },
      });
      this.setupFonts(doc);
      const chunks: Buffer[] = [];
      doc.on('data', (chunk: Buffer) => chunks.push(chunk));
  ...
  private buildCateringOrderPDF(doc: typeof PDFDocument, data: CateringOrderPDFData): void {
    // Nagłówek
    doc.fontSize(20).font('Roboto-Bold').text('ZAMÓWIENIE CATERINGOWE', 40, 40);
    doc.fontSize(10).font('Roboto').text(`Nr: ${data.orderNumber}`, 40, 70);
    doc.text(`Data utworzenia: ${new Date(data.createdAt).toLocaleDateString('pl-PL')}`, 40, 85);
    
    // Dane restauracji
    doc.fontSize(8).font('Roboto-Bold').text(this.restaurantData.name, 400, 40);
    doc.fontSize(8).font('Roboto').text(this.restaurantData.address ?? '', 400, 55);
    if (this.restaurantData.phone) doc.text(`Tel: ${this.restaurantData.phone}`, 400, 70);
    ...
    // Dane klienta
    doc.fontSize(12).font('Roboto-Bold').text('KLIENT', 40, y);
    y += 20;
    ...
    if (data.client.email) {
      doc.text(`Email: ${data.client.email}`, 40, y);
      y += 15;
    }
    if (data.client.phone) {
      doc.text(`Tel: ${data.client.phone}`, 40, y);
      y += 15;
    }
    if (data.client.address) {
      doc.text(`Adres: ${data.client.address}`, 40, y);
      y += 15;
    }
    ...
    doc.fontSize(12).font('Roboto-Bold').text('SZCZEGÓŁY WYDARZENIA', 40, y);
    y += 20;
    doc.fontSize(10).font('Roboto');
    doc.text(`Data: ${new Date(data.eventDate).toLocaleDateString('pl-PL')}`, 40, y);
    y += 15;
    doc.text(`Typ dostawy: ${data.deliveryType}`, 40, y);
    y += 15;
    if (data.deliveryAddress) {
      doc.text(`Adres dostawy: ${data.deliveryAddress}`, 40, y);
      y += 15;
    }
    doc.text(`Liczba gości: ${data.guests}`, 40, y);
    y += 20;
    
    // Pozycje zamówienia
    doc.fontSize(12).font('Roboto-Bold').text('POZYCJE ZAMÓWIENIA', 40, y);
    y += 20;
    
    // Nagłówki tabeli
    doc.fontSize(9).font('Roboto-Bold');
    doc.text('Pozycja', 40, y);
    doc.text('Ilość', 350, y);
    doc.text('Cena jedn.', 410, y);
    doc.text('Razem', 490, y);
    y += 15;
    ...
    data.items.forEach(item => {
      doc.text(item.dishName, 40, y, { width: 300 });
      doc.text(item.quantity.toString(), 350, y);
      doc.text(`${item.unitPrice.toFixed(2)} zł`, 410, y);
      doc.text(`${item.totalPrice.toFixed(2)} zł`, 490, y);
      y += 20;
    });
    ...
    doc.fontSize(10).font('Roboto');
    doc.text('Suma:', 410, y);
    doc.font('Roboto-Bold').text(`${data.subtotal.toFixed(2)} zł`, 490, y);
    y += 15;
    
    if (data.discountAmount && data.discountAmount > 0) {
      doc.font('Roboto').text('Rabat:', 410, y);
      doc.text(`-${data.discountAmount.toFixed(2)} zł`, 490, y);
      y += 15;
    }
    
    doc.font('Roboto-Bold').text('Razem:', 410, y);
    doc.fontSize(12).text(`${data.totalPrice.toFixed(2)} zł`, 490, y);
    y += 25;
    
    // Status
    doc.fontSize(10).font('Roboto').text(`Status: ${data.status}`, 40, y);
    
    // Uwagi
    if (data.notes) {
      y += 25;
      doc.fontSize(12).font('Roboto-Bold').text('UWAGI', 40, y);
      y += 20;
      doc.fontSize(10).font('Roboto').text(data.notes, 40, y, { width: 515 });
    }
  }
}
