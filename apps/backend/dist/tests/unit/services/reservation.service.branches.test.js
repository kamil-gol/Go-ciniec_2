...
describe('ReservationService — Branch Coverage', () => {
    ...
    describe('create — menu package', () => {
        it('should create with menu package and snapshot', async () => {
            ...
            await svc.createReservation(dto, UID);
        ...
        it('should throw when menu package not found', async () => {
            ...
            await expect(svc.createReservation(dto, UID)).rejects.toThrow('Selected menu package not found');
        });
        it('should throw when guests below minGuests', async () => {
            ...
            await expect(svc.createReservation(dto, UID)).rejects.toThrow(/at least 100 guests/);
        });
        it('should throw when guests above maxGuests', async () => {
            db.menuPackage.findUnique.mockResolvedValue({ ...MENU_PKG, maxGuests: 10 });
            const dto = { ...BASE_DTO, menuPackageId: 'pkg-001' };
            await expect(svc.createReservation(dto, UID)).rejects.toThrow(/maximum 10 guests/);
        });
        ...
        it('should throw when no prices and no menu package', async () => {
            ...
            await expect(svc.createReservation(dto, UID)).rejects.toThrow(/Price per adult and per child/);
        });
    });
    // ════════════════════════════════════
    ...
    describe('create — option edge cases', () => {
        ...
        it('should throw when option not found', async () => {
            ...
            await expect(svc.createReservation(dto, UID)).rejects.toThrow(/not found/);
        });
        it('should throw when option is inactive', async () => {
            ...
            await expect(svc.createReservation(dto, UID)).rejects.toThrow(/not active/);
        });
        it('should throw when exceeding maxQuantity', async () => {
            ...
            await expect(svc.createReservation(dto, UID)).rejects.toThrow(/Maximum 3/);
        });
        it('should throw when non-multiple option quantity > 1', async () => {
            ...
            await expect(svc.createReservation(dto, UID)).rejects.toThrow(/does not allow multiple/);
        });
    ...
    describe('create — edge cases', () => {
        it('should throw when percentage discount > 100%', async () => {
            ...
            await expect(svc.createReservation(dto, UID)).rejects.toThrow(/100%/);
        });
        it('should throw when fixed discount > totalPrice', async () => {
            ...
            await expect(svc.createReservation(dto, UID)).rejects.toThrow(/nie może przekroczyć ceny/);
        });
        it('should throw on invalid confirmation deadline', async () => {
            const dto = { ...BASE_DTO, confirmationDeadline: FUTURE };
            await expect(svc.createReservation(dto, UID)).rejects.toThrow(/Confirmation deadline/);
        });
        it('should throw when endDateTime before startDateTime', async () => {
            ...
            await expect(svc.createReservation(dto, UID)).rejects.toThrow('End time must be after start time');
        });
        it('should throw when client not found', async () => {
            db.client.findUnique.mockResolvedValue(null);
            await expect(svc.createReservation(BASE_DTO, UID)).rejects.toThrow('Client not found');
        });
        it('should throw when event type not found', async () => {
            db.eventType.findUnique.mockResolvedValue(null);
            await expect(svc.createReservation(BASE_DTO, UID)).rejects.toThrow('Event type not found');
        });
        it('should append next-year note for new format', async () => {
            await svc.createReservation(BASE_DTO, UID);
            const call = db.reservation.create.mock.calls[0][0];
            expect(call.data.notes).toContain('kolejny rok');
        });
    ...
    describe('create — whole venue conflicts', () => {
        it('should throw when booking whole venue but another hall is booked', async () => {
            ...
            await expect(svc.createReservation(dto, UID)).rejects.toThrow(/Nie można zarezerwować całego obiektu/);
        });
        it('should throw when regular hall but whole venue is booked', async () => {
            ...
            await expect(svc.createReservation(BASE_DTO, UID)).rejects.toThrow(/cały obiekt jest już zarezerwowany/);
        });
        ...
        it('should handle whole venue conflict with null client', async () => {
            ...
            const dto = { ...BASE_DTO, hallId: HALL_WHOLE.id };
            await expect(svc.createReservation(dto, UID)).rejects.toThrow(/nieznany klient/);
        });
        it('should handle non-whole-venue conflict with null client', async () => {
            ...
            await expect(svc.createReservation(BASE_DTO, UID)).rejects.toThrow(/nieznany klient/);
        });
    });
    // ════════════════════════════════════
    ...
    describe('updateReservationMenu()', () => {
        ...
        it('should remove menu (null menuPackageId) with existing snapshot', async () => {
            ...
            const result = await svc.updateReservationMenu('res-001', { menuPackageId: null }, UID);
        ...
        it('should update existing snapshot when changing package', async () => {
            db.reservation.findUnique.mockResolvedValue({ ...RES_BASE, menuSnapshot: SNAP });
            db.menuPackage.findUnique.mockResolvedValue(MENU_PKG);
            await svc.updateReservationMenu('res-001', { menuPackageId: 'pkg-001' }, UID);
        ...
        it('should throw on COMPLETED reservation', async () => {
            ...
            await expect(svc.updateReservationMenu('res-001', { menuPackageId: 'x' }, UID)).rejects.toThrow(/completed or cancelled/);
        });
        ...
        it('should throw when not found', async () => {
            ...
            await expect(svc.updateReservationMenu('bad', {}, UID)).rejects.toThrow('Reservation not found');
        });
        it('should throw when menu package not found', async () => {
            db.reservation.findUnique.mockResolvedValue(RES_BASE);
            db.menuPackage.findUnique.mockResolvedValue(null);
            await expect(svc.updateReservationMenu('res-001', { menuPackageId: 'bad' }, UID)).rejects.toThrow('Menu package not found');
        });
        it('should throw on invalid data (no menuPackageId key)', async () => {
            db.reservation.findUnique.mockResolvedValue(RES_BASE);
            await expect(svc.updateReservationMenu('res-001', {}, UID)).rejects.toThrow('Invalid menu update data');
        });
        it('should use custom adultsCount/childrenCount/toddlersCount', async () => {
            db.reservation.findUnique.mockResolvedValue(RES_BASE);
            db.menuPackage.findUnique.mockResolvedValue(MENU_PKG);
            await svc.updateReservationMenu('res-001', { menuPackageId: 'pkg-001', adultsCount: 30, childrenCount: 5, toddlersCount: 2 }, UID);
    ...
    describe('updateStatus() — branches', () => {
        it('should throw when completing before event date (startDateTime)', async () => {
            ...
            await expect(svc.updateStatus('res-001', { status: ReservationStatus.COMPLETED }, UID))
        ...
        it('should allow completing when event is in the past', async () => {
            ...
            await svc.updateStatus('res-001', { status: ReservationStatus.COMPLETED }, UID);
            expect(db.reservation.update).toHaveBeenCalledTimes(1);
        });
        ...
        it('should cancel via updateStatus with transaction', async () => {
            db.reservation.findUnique.mockResolvedValue({ ...RES_BASE, status: 'PENDING' });
            txMock.deposit.findMany.mockResolvedValue([]);
            await svc.updateStatus('res-001', { status: ReservationStatus.CANCELLED, reason: 'Anulacja' }, UID);
            expect(db.$transaction).toHaveBeenCalledTimes(1);
        });
        it('should cancel via updateStatus with reason in history', async () => {
            db.reservation.findUnique.mockResolvedValue({ ...RES_BASE, status: 'PENDING' });
            txMock.deposit.findMany.mockResolvedValue([]);
            await svc.updateStatus('res-001', { status: ReservationStatus.CANCELLED, reason: 'Powód anulacji' }, UID);
            const hist = txMock.reservationHistory.create.mock.calls[0][0];
        ...
        it('should cancel without reason', async () => {
            db.reservation.findUnique.mockResolvedValue({ ...RES_BASE, status: 'PENDING' });
            txMock.deposit.findMany.mockResolvedValue([]);
            await svc.updateStatus('res-001', { status: ReservationStatus.CANCELLED }, UID);
            const hist = txMock.reservationHistory.create.mock.calls[0][0];
            expect(hist.data.reason).toContain('Status changed');
        });
        it('should throw on invalid status transition', async () => {
            db.reservation.findUnique.mockResolvedValue({ ...RES_BASE, status: 'COMPLETED' });
            await expect(svc.updateStatus('res-001', { status: ReservationStatus.PENDING }, UID))
                .rejects.toThrow(/Cannot change status/);
        });
        it('should throw when reservation not found', async () => {
            db.reservation.findUnique.mockResolvedValue(null);
            await expect(svc.updateStatus('bad', { status: ReservationStatus.CONFIRMED }, UID)).rejects.toThrow('Reservation not found');
        });
        it('should perform regular update for non-cancel status change', async () => {
            db.reservation.findUnique.mockResolvedValue({ ...RES_BASE, status: 'PENDING' });
            await svc.updateStatus('res-001', { status: ReservationStatus.CONFIRMED }, UID);
            expect(db.reservation.update).toHaveBeenCalledTimes(1);
            expect(db.$transaction).not.toHaveBeenCalled();
        });
        it('should use reason in history for non-cancel status change', async () => {
            db.reservation.findUnique.mockResolvedValue({ ...RES_BASE, status: 'PENDING' });
            await svc.updateStatus('res-001', { status: ReservationStatus.CONFIRMED, reason: 'Klient potwierdził' }, UID);
...
//# sourceMappingURL=reservation.service.branches.test.js.map