...
describe('EventTypeService', () => {
    describe('createEventType()', () => {
        it('should create and audit', async () => {
            await service.createEventType({ name: 'Wesele' }, USER);
        ...
        it('should throw when name is empty', async () => {
            await expect(service.createEventType({ name: '  ' }, USER)).rejects.toThrow('Event type name is required');
        });
        it('should throw on invalid color', async () => {
            await expect(service.createEventType({ name: 'Test', color: 'red' }, USER)).rejects.toThrow('Invalid color format');
        });
        it('should throw on duplicate name', async () => {
            mockPrisma.eventType.findFirst.mockResolvedValue(ET);
            await expect(service.createEventType({ name: 'Wesele' }, USER)).rejects.toThrow('already exists');
        });
    });
    ...
    describe('updateEventType()', () => {
        it('should update and audit', async () => {
            await service.updateEventType('et-001', { name: 'Chrzciny' }, USER);
            expect(logChange).toHaveBeenCalledWith(expect.objectContaining({ action: 'UPDATE' }));
        });
        it('should throw when not found', async () => {
            mockPrisma.eventType.findUnique.mockResolvedValue(null);
            await expect(service.updateEventType('x', {}, USER)).rejects.toThrow('Event type not found');
        });
        it('should throw on empty name', async () => {
            await expect(service.updateEventType('et-001', { name: '' }, USER)).rejects.toThrow('name cannot be empty');
        });
        it('should throw on invalid color', async () => {
            await expect(service.updateEventType('et-001', { color: 'xyz' }, USER)).rejects.toThrow('Invalid color format');
        });
        it('should throw on duplicate name', async () => {
            mockPrisma.eventType.findFirst.mockResolvedValue({ id: 'et-other', name: 'Chrzciny' });
            await expect(service.updateEventType('et-001', { name: 'Chrzciny' }, USER)).rejects.toThrow('already exists');
        });
        it('should not audit when no changes', async () => {
            diffObjects.mockReturnValue({});
            await service.updateEventType('et-001', { name: 'Wesele' }, USER);
...
//# sourceMappingURL=eventType.service.test.js.map