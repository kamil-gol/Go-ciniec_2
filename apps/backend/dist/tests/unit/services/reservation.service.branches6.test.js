...
jest.mock('../../../utils/reservation.utils', () => ({
    calculateTotalGuests: jest.fn((a, c, t) => a + c + t),
    calculateTotalPrice: jest.fn((...args) => args[0] * args[2] + args[1] * args[3]),
...
const mockDiffObjects = diffObjects;
const mockDetectChanges = detectReservationChanges;
const mockFormatChanges = formatChangesSummary;
let service;
const UID = 'user-1';
const baseExisting = {
...
describe('updateReservation — manual price recalculation (no menu package)', () => {
    it('should recalculate totalPrice when adults change (no menu)', async () => {
        ...
        await service.updateReservation('res-1', { adults: 60 }, UID);
    ...
    it('should recalculate totalPrice when pricePerAdult changes (no menu, no guest change)', async () => {
        ...
        await service.updateReservation('res-1', { pricePerAdult: 250 }, UID);
        const updateCall = mockPrisma.reservation.update.mock.calls[0][0];
    ...
    it('should recalculate totalPrice when pricePerChild changes', async () => {
        ...
        await service.updateReservation('res-1', { pricePerChild: 120 }, UID);
        const updateCall = mockPrisma.reservation.update.mock.calls[0][0];
    ...
    it('should recalculate totalPrice when pricePerToddler changes', async () => {
        ...
        await service.updateReservation('res-1', { pricePerToddler: 50 }, UID);
        const updateCall = mockPrisma.reservation.update.mock.calls[0][0];
...
describe('updateReservation — detectedChanges triggers history + audit', () => {
    ...
    it('should throw when changes detected but reason too short', async () => {
        ...
        mockPrisma.reservation.findUnique.mockResolvedValue(baseExisting);
        await expect(service.updateReservation('res-1', {
            adults: 60,
            reason: 'short',
        }, UID)).rejects.toThrow('Reason is required for changes (minimum 10 characters)');
    });
    it('should throw when changes detected but no reason provided', async () => {
        ...
        mockPrisma.reservation.findUnique.mockResolvedValue(baseExisting);
        await expect(service.updateReservation('res-1', { adults: 60 }, UID)).rejects.toThrow('Reason is required for changes (minimum 10 characters)');
    });
});
//# sourceMappingURL=reservation.service.branches6.test.js.map