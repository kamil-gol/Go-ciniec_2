...
describe('QueueService', () => {
    ...
    describe('moveToPosition()', () => {
        it('should move reservation via $executeRaw and audit', async () => {
            ...
            expect(mockPrisma.$executeRaw).toHaveBeenCalledTimes(1);
            expect(logChange).toHaveBeenCalledWith(expect.objectContaining({ action: 'QUEUE_MOVE' }));
        });
        ...
        it('should throw when reservation is not RESERVED', async () => {
            mockPrisma.reservation.findUnique.mockResolvedValue(makeReservation('res-001', 1, { status: 'PENDING' }));
    ...
    describe('batchUpdatePositions()', () => {
        it('should batch update positions in transaction', async () => {
            ...
            expect(logChange).toHaveBeenCalledWith(expect.objectContaining({ action: 'QUEUE_REORDER' }));
        });
    ...
    describe('rebuildPositions()', () => {
        it('should rebuild positions ordered by createdAt per date', async () => {
            ...
            expect(logChange).toHaveBeenCalledWith(expect.objectContaining({ action: 'QUEUE_REBUILD' }));
        });
    ...
    describe('promoteReservation()', () => {
        ...
        it('should promote RESERVED to PENDING with calculated price', async () => {
            ...
            const result = await service.promoteReservation('res-001', PROMOTE_DATA, TEST_USER_ID);
            ...
            expect(logChange).toHaveBeenCalledWith(expect.objectContaining({ action: 'QUEUE_PROMOTE' }));
        });
        it('should throw when reservation not found', async () => {
            mockPrisma.reservation.findUnique.mockResolvedValue(null);
            await expect(service.promoteReservation('res-999', PROMOTE_DATA, TEST_USER_ID))
                .rejects.toThrow('Reservation not found');
        });
        it('should throw when reservation is not RESERVED', async () => {
            mockPrisma.reservation.findUnique.mockResolvedValue(makeReservation('res-001', 1, { status: 'CONFIRMED' }));
            await expect(service.promoteReservation('res-001', PROMOTE_DATA, TEST_USER_ID))
        ...
        it('should throw when required fields are missing', async () => {
            ...
            }, TEST_USER_ID)).rejects.toThrow('Hall, event type, start time, and end time are required');
        });
...
//# sourceMappingURL=queue.service.business.test.js.map