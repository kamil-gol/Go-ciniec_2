...
describe('MenuService', () => {
    ...
    describe('updateMenuTemplate()', () => {
        it('should update and audit when changes detected', async () => {
            const result = await service.updateMenuTemplate('tpl-001', { name: 'Updated' }, TEST_USER);
        ...
        it('should NOT audit when no changes', async () => {
            ...
            await service.updateMenuTemplate('tpl-001', { name: 'Same' }, TEST_USER);
            expect(logChange).not.toHaveBeenCalled();
        });
        it('should throw when not found', async () => {
            mockPrisma.menuTemplate.findUnique.mockResolvedValue(null);
            await expect(service.updateMenuTemplate('nonexistent', {}, TEST_USER)).rejects.toThrow('Menu template not found');
        });
    });
    ...
    describe('updatePackage()', () => {
        ...
        it('should throw when not found', async () => {
            mockPrisma.menuPackage.findUnique.mockResolvedValue(null);
            await expect(service.updatePackage('nonexistent', {}, TEST_USER)).rejects.toThrow('Package not found');
        });
    });
...
//# sourceMappingURL=menu.service.templates.test.js.map