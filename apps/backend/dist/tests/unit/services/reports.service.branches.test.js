...
const setupRevenue = (reservations = [makeReservation()]) => {
...
describe('ReportsService — branches', () => {
    ...
    describe('getRevenueReport()', () => {
        ...
        it('should return report with default groupBy=month', async () => {
            setupRevenue();
            const result = await reportsService.getRevenueReport(baseFilters);
        ...
        it('should filter by hallId', async () => {
            setupRevenue();
            await reportsService.getRevenueReport({ ...baseFilters, hallId: 'h-1' });
        ...
        it('should filter by eventTypeId', async () => {
            setupRevenue();
            await reportsService.getRevenueReport({ ...baseFilters, eventTypeId: 'et-1' });
            const where = db.reservation.findMany.mock.calls[0][0].where;
            expect(where.eventTypeId).toBe('et-1');
        });
        it('should filter by status', async () => {
            setupRevenue();
            await reportsService.getRevenueReport({ ...baseFilters, status: 'COMPLETED' });
            const where = db.reservation.findMany.mock.calls[0][0].where;
            expect(where.status).toBe('COMPLETED');
        });
        it('should not filter when no optional filters', async () => {
            setupRevenue();
            await reportsService.getRevenueReport(baseFilters);
            const where = db.reservation.findMany.mock.calls[0][0].where;
        ...
        it('should group by day', async () => {
            ...
            const result = await reportsService.getRevenueReport({ ...baseFilters, groupBy: 'day' });
            expect(result.breakdown[0].period).toBe('2026-02-16');
        });
        it('should group by week', async () => {
            setupRevenue([makeReservation({ date: '2026-02-16' })]);
            const result = await reportsService.getRevenueReport({ ...baseFilters, groupBy: 'week' });
            expect(result.breakdown[0].period).toMatch(/2026-W/);
        });
        it('should group by month', async () => {
            setupRevenue([makeReservation({ date: '2026-02-16' })]);
            const result = await reportsService.getRevenueReport({ ...baseFilters, groupBy: 'month' });
            expect(result.breakdown[0].period).toBe('2026-02');
        });
        it('should group by year', async () => {
            setupRevenue([makeReservation({ date: '2026-02-16' })]);
            const result = await reportsService.getRevenueReport({ ...baseFilters, groupBy: 'year' });
    ...
    describe('getOccupancyReport()', () => {
        ...
        const setupOccupancy = (reservations = [makeReservation()]) => {
            db.reservation.findMany.mockResolvedValue(reservations);
        };
        it('should return basic occupancy report', async () => {
            setupOccupancy();
            const result = await reportsService.getOccupancyReport(baseFilters);
            expect(result.summary.totalReservations).toBe(1);
            expect(result.summary.totalDaysInPeriod).toBe(31);
        });
        it('should filter by hallId', async () => {
            setupOccupancy();
            await reportsService.getOccupancyReport({ ...baseFilters, hallId: 'h-1' });
            const where = db.reservation.findMany.mock.calls[0][0].where;
            expect(where.hallId).toBe('h-1');
        });
        it('should handle empty reservations', async () => {
        ...
        it('should limit peakHours to top 10', async () => {
            const reservations = Array.from({ length: 15 }, (_, i) => makeReservation({ id: `r-${i}`, startTime: `${(i % 24).toString().padStart(2, '0')}:00` }));
...
//# sourceMappingURL=reports.service.branches.test.js.map