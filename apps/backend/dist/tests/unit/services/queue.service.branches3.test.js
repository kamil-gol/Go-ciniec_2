...
describe('QueueService — 55P03 lock error branch in withRetry (line 39)', () => {
    it('swapPositions retries on error with 55P03 in message then throws', async () => {
        ...
        await expect(service.swapPositions('r1', 'r2', 'user-1')).rejects.toThrow('Another user is modifying the queue');
        // Should have been called 3 times (initial + 2 retries)
        expect(mockPrisma.$executeRaw).toHaveBeenCalledTimes(3);
    }, 10000);
});
//# sourceMappingURL=queue.service.branches3.test.js.map