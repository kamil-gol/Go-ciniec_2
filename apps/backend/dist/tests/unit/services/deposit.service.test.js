...
describe('DepositService', () => {
    ...
    describe('create', () => {
        ...
        it('should create deposit and log activity', async () => {
            ...
            expect(mockPrisma.$queryRawUnsafe).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO "Deposit"'), 'res-1', 2000, 2000, '2026-05-01');
            expect(logChange).toHaveBeenCalledWith(expect.objectContaining({ action: 'CREATE', entityType: 'DEPOSIT' }));
        });
    ...
    describe('update', () => {
        it('should update amount and dueDate together', async () => {
            ...
            expect(mockPrisma.$queryRawUnsafe).toHaveBeenCalledWith(expect.stringContaining('amount'), 3000, 3000, '2026-06-01', 'dep-1');
            expect(logChange).toHaveBeenCalledWith(expect.objectContaining({ action: 'UPDATE', entityType: 'DEPOSIT' }));
        });
        it('should update only dueDate when amount is not provided', async () => {
            ...
            expect(mockPrisma.$queryRawUnsafe).toHaveBeenCalledWith(expect.stringContaining('dueDate'), '2026-07-01', 'dep-1');
        });
    ...
    describe('delete', () => {
        it('should delete unpaid deposit and log activity', async () => {
            ...
            expect(mockPrisma.$queryRawUnsafe).toHaveBeenCalledWith(expect.stringContaining('DELETE'), 'dep-1');
            expect(logChange).toHaveBeenCalledWith(expect.objectContaining({ action: 'DELETE', entityType: 'DEPOSIT' }));
        });
    ...
    describe('markAsPaid', () => {
        ...
        it('should mark deposit as fully PAID', async () => {
            ...
            expect(mockPrisma.$queryRawUnsafe).toHaveBeenCalledWith(expect.stringContaining('UPDATE "Deposit"'), true, 'PAID', '2026-04-20T12:00:00Z', 'TRANSFER', 0, 2000, 'dep-1');
            expect(logChange).toHaveBeenCalledWith(expect.objectContaining({ action: 'MARK_PAID' }));
        });
        it('should mark as PARTIALLY_PAID when amountPaid < total', async () => {
            ...
            expect(mockPrisma.$queryRawUnsafe).toHaveBeenCalledWith(expect.stringContaining('UPDATE "Deposit"'), false, 'PARTIALLY_PAID', '2026-04-20T12:00:00Z', 'TRANSFER', 1000, 1000, 'dep-1');
        });
    ...
    describe('checkAndAutoConfirmReservation', () => {
        it('should auto-confirm PENDING reservation when all deposits are PAID', async () => {
            ...
            expect(mockPrisma.reservationHistory.create).toHaveBeenCalledWith(expect.objectContaining({
    ...
    describe('sendConfirmationEmail', () => {
        it('should send email with PDF for paid deposit', async () => {
            ...
            expect(mockEmailService.sendDepositPaidConfirmation).toHaveBeenCalledWith('jan@test.pl', expect.objectContaining({ clientName: 'Jan Kowalski' }), expect.any(Buffer));
        });
    ...
    describe('markAsUnpaid', () => {
        it('should reset paid deposit to PENDING', async () => {
            ...
            expect(mockPrisma.$queryRawUnsafe).toHaveBeenCalledWith(expect.stringContaining("status = 'PENDING'"), 2000, 'dep-paid');
            expect(logChange).toHaveBeenCalledWith(expect.objectContaining({ action: 'MARK_UNPAID' }));
        });
    ...
    describe('cancel', () => {
        it('should cancel unpaid deposit', async () => {
            ...
            expect(mockPrisma.$queryRawUnsafe).toHaveBeenCalledWith(expect.stringContaining("'CANCELLED'"), 'dep-1');
            expect(logChange).toHaveBeenCalledWith(expect.objectContaining({ action: 'CANCEL', entityType: 'DEPOSIT' }));
        });
    ...
    describe('autoMarkOverdue', () => {
        it('should batch-update overdue deposits and return count', async () => {
            ...
            expect(mockPrisma.$queryRawUnsafe).toHaveBeenCalledWith(expect.stringContaining('UPDATE "Deposit"'), expect.any(String));
        });
...
//# sourceMappingURL=deposit.service.test.js.map