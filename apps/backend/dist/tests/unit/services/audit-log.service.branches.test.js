...
describe('AuditLogService — branches', () => {
    ...
    describe('getAuditLogs()', () => {
        ...
        it('should filter by entityType (not ALL)', async () => {
            ...
            expect(db.activityLog.count).toHaveBeenCalledWith(expect.objectContaining({ where: expect.objectContaining({ entityType: 'RESERVATION' }) }));
        });
        ...
        it('should filter by action (not ALL)', async () => {
            ...
            expect(db.activityLog.count).toHaveBeenCalledWith(expect.objectContaining({ where: expect.objectContaining({ action: 'CREATE' }) }));
        });
        ...
        it('should filter by userId', async () => {
            ...
            expect(db.activityLog.count).toHaveBeenCalledWith(expect.objectContaining({ where: expect.objectContaining({ userId: 'u-1' }) }));
        });
        it('should filter by entityId', async () => {
            setup();
            await auditLogService.getAuditLogs({ entityId: 'e-1' });
            expect(db.activityLog.count).toHaveBeenCalledWith(expect.objectContaining({ where: expect.objectContaining({ entityId: 'e-1' }) }));
        });
    ...
    describe('getRecentActivity()', () => {
        it('should use default limit of 10', async () => {
            ...
            expect(db.activityLog.findMany).toHaveBeenCalledWith(expect.objectContaining({ take: 10 }));
        });
        it('should use custom limit', async () => {
            db.activityLog.findMany.mockResolvedValue([]);
            await auditLogService.getRecentActivity(5);
            expect(db.activityLog.findMany).toHaveBeenCalledWith(expect.objectContaining({ take: 5 }));
        });
    });
    // ═══ getActions ═══
    describe('getActions()', () => {
...
//# sourceMappingURL=audit-log.service.branches.test.js.map