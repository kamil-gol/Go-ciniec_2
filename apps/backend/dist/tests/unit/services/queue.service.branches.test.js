...
const makeReservation = (id, position, overrides) => ({
    id, status: 'RESERVED',
...
beforeEach(() => {
    ...
    mockPrisma.reservation.update.mockImplementation(({ where }) => {
        if (where.id === 'res-001')
            return Promise.resolve(RES_1);
...
describe('QueueService — branch coverage', () => {
    ...
    describe('addToQueue — defaults', () => {
        it('should default adults to guests when adults not provided', async () => {
            ...
            mockPrisma.reservation.create.mockResolvedValue(makeReservation('res-new', 1, { adults: 30, children: 0, toddlers: 0 }));
        ...
        it('should use provided adults/children/toddlers values', async () => {
            mockPrisma.reservation.aggregate.mockResolvedValue({ _max: { reservationQueuePosition: 0 } });
            mockPrisma.reservation.create.mockResolvedValue(makeReservation('res-new', 1, { adults: 20, children: 5, toddlers: 3 }));
            await service.addToQueue({
        ...
        it('should throw on P2002 unique constraint error during create', async () => {
            ...
            }, TEST_USER_ID)).rejects.toThrow('Position 4 is already taken');
        });
        it('should re-throw non-P2002 errors from create', async () => {
            ...
            await expect(service.addToQueue({
                clientId: 'client-001', reservationQueueDate: '2026-09-15', guests: 10,
            }, TEST_USER_ID)).rejects.toThrow('DB connection lost');
        });
    ...
    describe('swapPositions — error branches', () => {
        it('should throw user-friendly message on lock error from $executeRaw', async () => {
            ...
            mockPrisma.$executeRaw.mockRejectedValue(Object.assign(new Error('lock_not_available'), { code: undefined }));
        ...
        it('should throw user-friendly message on P2034 error', async () => {
            mockPrisma.reservation.findUnique
                .mockResolvedValueOnce(RES_1)
                .mockResolvedValueOnce(RES_2);
            mockPrisma.$executeRaw.mockRejectedValue(Object.assign(new Error('Transaction failed'), { code: 'P2034' }));
            await expect(service.swapPositions('res-001', 'res-002', TEST_USER_ID))
                .rejects.toThrow('Another user is modifying the queue');
        });
        it('should throw on P2002 error during swap', async () => {
            mockPrisma.reservation.findUnique
                .mockResolvedValueOnce(RES_1)
                .mockResolvedValueOnce(RES_2);
            mockPrisma.$executeRaw.mockRejectedValue(Object.assign(new Error('Unique constraint'), { code: 'P2002' }));
            await expect(service.swapPositions('res-001', 'res-002', TEST_USER_ID))
    ...
    describe('moveToPosition — error branches', () => {
        ...
        it('should throw when reservation has no queue date', async () => {
            mockPrisma.reservation.findUnique.mockResolvedValue(makeReservation('res-001', 1, { reservationQueueDate: null }));
    ...
    describe('updateQueueReservation — branches', () => {
        ...
        it('should throw when reservation is not RESERVED', async () => {
            mockPrisma.reservation.findUnique.mockResolvedValue(makeReservation('res-001', 1, { status: 'CONFIRMED' }));
        ...
        it('should throw when guests < 1 during update', async () => {
            mockPrisma.reservation.findUnique.mockResolvedValue(RES_1);
            await expect(service.updateQueueReservation('res-001', { guests: 0 }, TEST_USER_ID))
                .rejects.toThrow('Number of guests must be at least 1');
        });
        it('should throw on invalid queue date during update', async () => {
            mockPrisma.reservation.findUnique.mockResolvedValue(RES_1);
            await expect(service.updateQueueReservation('res-001', { reservationQueueDate: 'bad-date' }, TEST_USER_ID))
                .rejects.toThrow('Invalid queue date format');
        });
        it('should throw when queue date is in the past during update', async () => {
            mockPrisma.reservation.findUnique.mockResolvedValue(RES_1);
            await expect(service.updateQueueReservation('res-001', { reservationQueueDate: '2020-01-01' }, TEST_USER_ID))
                .rejects.toThrow('Queue date cannot be in the past');
        });
        it('should reposition when date changes', async () => {
            mockPrisma.reservation.findUnique.mockResolvedValue(RES_1);
            mockPrisma.reservation.aggregate.mockResolvedValue({ _max: { reservationQueuePosition: 3 } });
            mockPrisma.reservation.update.mockResolvedValue(makeReservation('res-001', 4, { reservationQueueDate: new Date('2026-10-01') }));
        ...
        it('should track clientId change', async () => {
            ...
            mockPrisma.reservation.update.mockResolvedValue(makeReservation('res-001', 1, { clientId: 'client-002', client: newClient }));
            await service.updateQueueReservation('res-001', { clientId: 'client-002' }, TEST_USER_ID);
            const { logChange } = require('../../../utils/audit-logger');
            expect(logChange).toHaveBeenCalledWith(expect.objectContaining({ action: 'QUEUE_UPDATE' }));
        });
        it('should not audit when no changes detected', async () => {
            mockPrisma.reservation.findUnique.mockResolvedValue(RES_1);
            mockPrisma.reservation.update.mockResolvedValue(RES_1);
            await service.updateQueueReservation('res-001', { notes: RES_1.notes }, TEST_USER_ID);
            const { logChange } = require('../../../utils/audit-logger');
        ...
        it('should track notes change when notes differ', async () => {
            ...
            await service.updateQueueReservation('res-001', { notes: 'new' }, TEST_USER_ID);
            const { logChange } = require('../../../utils/audit-logger');
        ...
        it('should convert empty string notes to null', async () => {
            ...
            await service.updateQueueReservation('res-001', { notes: '' }, TEST_USER_ID);
    ...
    describe('promoteReservation — extra branches', () => {
        it('should throw on invalid date format', async () => {
            ...
            }, TEST_USER_ID)).rejects.toThrow('Invalid date/time format');
        });
        it('should throw when end time is before start time', async () => {
            ...
            await expect(service.promoteReservation('res-001', {
                ...
                adults: 10, pricePerAdult: 100,
            }, TEST_USER_ID)).rejects.toThrow('End time must be after start time');
        });
        ...
        it('should handle promote when reservation has no queue date/position', async () => {
            mockPrisma.reservation.findUnique.mockResolvedValue(makeReservation('res-001', 1, { reservationQueueDate: null, reservationQueuePosition: null }));
            mockPrisma.reservation.findFirst.mockResolvedValue(null);
            mockPrisma.reservation.update.mockResolvedValue({
...
//# sourceMappingURL=queue.service.branches.test.js.map