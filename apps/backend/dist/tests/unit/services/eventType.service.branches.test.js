...
describe('EventTypeService — branches', () => {
    ...
    describe('createEventType()', () => {
        it('should throw when name is empty', async () => {
            await expect(svc.createEventType({ name: '' }, 'u-1')).rejects.toThrow('required');
        });
        ...
        it('should default description to null', async () => {
            ...
            await svc.createEventType({ name: 'New' }, 'u-1');
            expect(db.eventType.create).toHaveBeenCalledWith(expect.objectContaining({ data: expect.objectContaining({ description: null }) }));
        });
        it('should default color to null', async () => {
            db.eventType.findFirst.mockResolvedValue(null);
            db.eventType.create.mockResolvedValue(makeET());
            await svc.createEventType({ name: 'New' }, 'u-1');
            expect(db.eventType.create).toHaveBeenCalledWith(expect.objectContaining({ data: expect.objectContaining({ color: null }) }));
        });
        it('should default isActive to true', async () => {
            db.eventType.findFirst.mockResolvedValue(null);
            db.eventType.create.mockResolvedValue(makeET());
            await svc.createEventType({ name: 'New' }, 'u-1');
            expect(db.eventType.create).toHaveBeenCalledWith(expect.objectContaining({ data: expect.objectContaining({ isActive: true }) }));
        });
        it('should use explicit isActive=false', async () => {
            db.eventType.findFirst.mockResolvedValue(null);
            db.eventType.create.mockResolvedValue(makeET({ isActive: false }));
            await svc.createEventType({ name: 'New', isActive: false }, 'u-1');
            expect(db.eventType.create).toHaveBeenCalledWith(expect.objectContaining({ data: expect.objectContaining({ isActive: false }) }));
        });
        it('should use provided description and color', async () => {
            db.eventType.findFirst.mockResolvedValue(null);
            db.eventType.create.mockResolvedValue(makeET());
            await svc.createEventType({ name: 'New', description: 'Desc', color: '#00FF00' }, 'u-1');
            expect(db.eventType.create).toHaveBeenCalledWith(expect.objectContaining({
    ...
    describe('getEventTypes()', () => {
        it('should return all when activeOnly=false', async () => {
            ...
            expect(db.eventType.findMany).toHaveBeenCalledWith(expect.objectContaining({ where: {} }));
        });
        it('should filter active when activeOnly=true', async () => {
            db.eventType.findMany.mockResolvedValue([]);
            await svc.getEventTypes(true);
            expect(db.eventType.findMany).toHaveBeenCalledWith(expect.objectContaining({ where: { isActive: true } }));
        });
    ...
    describe('updateEventType()', () => {
        it('should throw when not found', async () => {
            ...
            await expect(svc.updateEventType('bad', { name: 'X' }, 'u-1')).rejects.toThrow('not found');
        });
        ...
        it('should allow color=null (remove color)', async () => {
            ...
            await svc.updateEventType('et-1', { color: null }, 'u-1');
    ...
    describe('toggleActive()', () => {
        it('should throw when not found', async () => {
            db.eventType.findUnique.mockResolvedValue(null);
            await expect(svc.toggleActive('bad', 'u-1')).rejects.toThrow('not found');
        });
    ...
    describe('deleteEventType()', () => {
        it('should throw when not found', async () => {
            db.eventType.findUnique.mockResolvedValue(null);
            await expect(svc.deleteEventType('bad', 'u-1')).rejects.toThrow('not found');
        });
    ...
    describe('getEventTypeById()', () => {
        it('should throw when not found', async () => {
            db.eventType.findUnique.mockResolvedValue(null);
            await expect(svc.getEventTypeById('bad')).rejects.toThrow('not found');
        });
    });
    // ═══ getPredefinedColors ═══
...
//# sourceMappingURL=eventType.service.branches.test.js.map