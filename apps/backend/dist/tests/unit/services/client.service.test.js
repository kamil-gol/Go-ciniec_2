...
describe('ClientService', () => {
    ...
    describe('createClient()', () => {
        it('should throw on invalid email', async () => {
            await expect(svc.createClient({ firstName: 'Jan', lastName: 'K', phone: '123456789', email: 'invalid' }, 'u1')).rejects.toThrow('Invalid email format');
        });
        it('should throw when phone is missing', async () => {
            await expect(svc.createClient({ firstName: 'Jan', lastName: 'K', phone: '' }, 'u1')).rejects.toThrow('Phone number is required');
        });
        it('should throw when phone has less than 9 digits', async () => {
            await expect(svc.createClient({ firstName: 'Jan', lastName: 'K', phone: '12345' }, 'u1')).rejects.toThrow('at least 9 digits');
        });
        it('should throw on duplicate client', async () => {
            db.client.findFirst.mockResolvedValue({ id: 'existing' });
            await expect(svc.createClient({ firstName: 'Jan', lastName: 'K', phone: '123456789' }, 'u1')).rejects.toThrow('ju\u017c istnieje');
        });
        it('should create client without email/notes', async () => {
            ...
            const result = await svc.createClient({ firstName: 'Jan', lastName: 'K', phone: '123456789' }, 'u1');
        ...
        it('should create client with email and notes', async () => {
            ...
            const result = await svc.createClient({ firstName: 'Jan', lastName: 'K', phone: '123456789', email: 'jan@test.pl', notes: 'VIP' }, 'u1');
        ...
        it('should accept valid email', async () => {
            ...
            const result = await svc.createClient({ firstName: 'A', lastName: 'B', phone: '123456789', email: 'a@b.com' }, 'u1');
    ...
    describe('updateClient()', () => {
        ...
        it('should set email to null when empty string', async () => {
            ...
            diffObjects.mockReturnValue({ email: { old: 'j@t.pl', new: null } });
            await svc.updateClient('c1', { email: '' }, 'u1');
            const call = db.client.update.mock.calls[0][0];
        ...
        it('should audit log when changes exist', async () => {
            db.client.findUnique.mockResolvedValue(EXISTING);
            db.client.update.mockResolvedValue({ ...EXISTING, firstName: 'Anna' });
            diffObjects.mockReturnValue({ firstName: { old: 'Jan', new: 'Anna' } });
            await svc.updateClient('c1', { firstName: 'Anna' }, 'u1');
