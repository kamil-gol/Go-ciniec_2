...
describe('AttachmentController — error branches', () => {
    ...
    it('getByEntity — withClientRodo=true and entityType=CLIENT skips cross-reference', async () => {
        ...
        const response = res();
        await attachmentController.getByEntity(req({ query: { entityType: 'CLIENT', entityId: 'c-1', withClientRodo: 'true' } }), response, next);
    ...
    it('update — calls next(error) on service failure', async () => {
        ...
        const response = res();
        await attachmentController.update(req({ params: { id: 'a-1' }, body: { label: 'X' } }), response, next);
        expect(next).toHaveBeenCalledWith(expect.any(Error));
    });
    ...
    describe('archive()', () => {
        it('should return archived attachment on success', async () => {
            ...
            expect(response.json).toHaveBeenCalledWith(expect.objectContaining({ data: expect.objectContaining({ id: 'a-1' }) }));
...
//# sourceMappingURL=attachment.controller.branches.test.js.map