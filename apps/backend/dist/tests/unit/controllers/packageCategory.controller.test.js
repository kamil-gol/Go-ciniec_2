...
const safeParse = bulkUpdateCategorySettingsSchema.safeParse;
const req = (overrides = {}) => ({
    body: {}, params: {}, query: {},
    ...overrides,
});
...
describe('PackageCategoryController', () => {
    ...
    describe('getByPackage()', () => {
        ...
        it('should return categories with dishes', async () => {
            ...
            expect(response.json).toHaveBeenCalledWith(expect.objectContaining({ success: true, data: expect.objectContaining({ packageName: 'Gold' }) }));
        });
    ...
    describe('getById()', () => {
        ...
        it('should return setting', async () => {
            ...
            expect(response.json).toHaveBeenCalledWith(expect.objectContaining({ success: true, data: { id: 'cs-1' } }));
        });
        it('should return 500 on error', async () => {
    ...
    describe('create()', () => {
        ...
        it('should return 201 with explicit values', async () => {
            ...
            expect(response.status).toHaveBeenCalledWith(201);
            expect(mockPrisma.packageCategorySettings.create).toHaveBeenCalledWith(expect.objectContaining({ data: expect.objectContaining({ isRequired: false, isEnabled: false, customLabel: 'Custom' }) }));
        });
        it('should return 500 on error', async () => {
            mockPrisma.menuPackage.findUnique.mockRejectedValue(new Error('db'));
            const response = res();
    ...
    describe('bulkUpdate()', () => {
        ...
        it('should return 200 with empty settings', async () => {
            ...
            expect(response.json).toHaveBeenCalledWith(expect.objectContaining({ success: true, data: [] }));
        });
        ...
        it('should use defaults for optional fields', async () => {
            ...
            const response = res();
            await ctrl.bulkUpdate(req({ params: { packageId: 'pkg-1' } }), response);
            expect(mockPrisma.packageCategorySettings.create).toHaveBeenCalledWith(expect.objectContaining({ data: expect.objectContaining({ isRequired: true, isEnabled: true, displayOrder: 0, customLabel: null }) }));
        });
        it('should return 500 on error', async () => {
            safeParse.mockReturnValue({ success: true, data: { settings: [] } });
            mockPrisma.menuPackage.findUnique.mockRejectedValue(new Error('db'));
...
//# sourceMappingURL=packageCategory.controller.test.js.map