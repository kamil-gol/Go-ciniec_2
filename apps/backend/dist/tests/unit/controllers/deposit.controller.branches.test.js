...
jest.mock('../../../validation/deposit.validation', () => ({
    depositFiltersSchema: { parse: jest.fn((x) => x) },
    createDepositSchema: { parse: jest.fn((x) => x) },
    updateDepositSchema: { parse: jest.fn((x) => x) },
    markPaidSchema: { parse: jest.fn((x) => x) },
}));
...
describe('Deposit Controller branches', () => {
    ...
    describe('downloadDepositPdf', () => {
        it('should throw when deposit not paid', async () => {
            depositService.getById.mockResolvedValue({ id: 'd1', paid: false });
        ...
        it('should throw when no reservation', async () => {
            depositService.getById.mockResolvedValue({ id: 'd1', paid: true, reservation: null });
            const req = { params: { id: 'd1' } };
            await expect(ctrl.downloadDepositPdf(req, mockRes())).rejects.toThrow('danych');
        });
        it('should throw when no client', async () => {
            depositService.getById.mockResolvedValue({
                id: 'd1', paid: true,
        ...
        it('should generate PDF with all fields present', async () => {
            ...
            pdfService.generatePaymentConfirmationPDF.mockResolvedValue(Buffer.from('pdf'));
            const req = { params: { id: 'd1' } };
            const res = mockRes();
            await ctrl.downloadDepositPdf(req, res);
            expect(res.setHeader).toHaveBeenCalledWith('Content-Type', 'application/pdf');
            expect(pdfService.generatePaymentConfirmationPDF).toHaveBeenCalledWith(expect.objectContaining({
                paymentMethod: 'CASH',
    ...
    describe('createDeposit', () => {
        it('should throw unauthorized when no userId', async () => {
            const req = { params: { reservationId: 'r1' }, body: { amount: 500, dueDate: '2026-03-01' }, user: undefined };
            await expect(ctrl.createDeposit(req, mockRes())).rejects.toThrow();
        });
        it('should create deposit', async () => {
            depositService.create.mockResolvedValue({ id: 'd1' });
            const req = { params: { reservationId: 'r1' }, body: { amount: 500, dueDate: '2026-03-01' }, user: { id: 'u1' } };
            const res = mockRes();
    ...
    describe('sendDepositEmail', () => {
        it('should send email', async () => {
            depositService.sendConfirmationEmail.mockResolvedValue({ success: true });
            const req = { params: { id: 'd1' } };
            const res = mockRes();
...
//# sourceMappingURL=deposit.controller.branches.test.js.map