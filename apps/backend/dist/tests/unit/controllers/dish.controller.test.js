...
describe('DishController', () => {
    describe('getDishes()', () => {
        it('should return dishes with filters', async () => {
            ...
            expect(svc.findAll).toHaveBeenCalledWith(expect.objectContaining({ categoryId: 'cat-1', isActive: true, search: 'zupa' }));
    ...
    describe('createDish()', () => {
        it('should throw 401 when no user', async () => {
            await expect(dishController.createDish(req({ user: undefined, body: { name: 'Zupa', categoryId: 'cat-1' } }), res())).rejects.toMatchObject({ statusCode: 401 });
        });
        it('should throw 400 when name missing', async () => {
            await expect(dishController.createDish(req({ body: { categoryId: 'cat-1' } }), res())).rejects.toMatchObject({ statusCode: 400 });
        });
        it('should return 201 on success', async () => {
            svc.create.mockResolvedValue({ id: 'd-new', name: 'Zupa pomidorowa' });
            const response = res();
            await dishController.createDish(req({ body: { name: 'Zupa pomidorowa', categoryId: 'cat-1' } }), response);
            expect(response.status).toHaveBeenCalledWith(201);
        });
    });
    describe('updateDish()', () => {
        it('should throw 401 when no user', async () => {
            await expect(dishController.updateDish(req({ user: undefined, params: { id: 'd-1' } }), res())).rejects.toMatchObject({ statusCode: 401 });
        });
    ...
    describe('deleteDish()', () => {
        it('should throw 401 when no user', async () => {
            await expect(dishController.deleteDish(req({ user: undefined, params: { id: 'd-1' } }), res())).rejects.toMatchObject({ statusCode: 401 });
        });
        it('should return 200 on success', async () => {
...
//# sourceMappingURL=dish.controller.test.js.map