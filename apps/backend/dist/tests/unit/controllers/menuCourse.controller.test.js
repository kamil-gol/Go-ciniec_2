...
describe('MenuCourseController', () => {
    ...
    describe('assignDishes()', () => {
        it('should return 200', async () => {
            ...
            const response = res();
            await controller.assignDishes(req({ params: { id: 'c-1' }, body: { dishes: [{ dishId: 'd-1' }] } }), response, next);
            expect(response.status).toHaveBeenCalledWith(200);
        });
    });
    describe('removeDish()', () => {
        it('should return 404 when not assigned', async () => {
            ...
            const response = res();
            await controller.removeDish(req({ params: { courseId: 'c-1', dishId: 'd-x' } }), response, next);
            expect(response.status).toHaveBeenCalledWith(404);
        });
        it('should return 200', async () => {
            svc.removeDish.mockResolvedValue(undefined);
            const response = res();
            await controller.removeDish(req({ params: { courseId: 'c-1', dishId: 'd-1' } }), response, next);
            expect(response.status).toHaveBeenCalledWith(200);
        });
    });
});
//# sourceMappingURL=menuCourse.controller.test.js.map