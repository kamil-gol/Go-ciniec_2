...
describe('QueueController', () => {
    describe('addToQueue()', () => {
        ...
        it('should return 201 on success', async () => {
            ...
            expect(response.json).toHaveBeenCalledWith(expect.objectContaining({ success: true, data: expect.objectContaining({ position: 3 }) }));
            expect(svc.addToQueue).toHaveBeenCalledWith(expect.objectContaining({ clientId: 'c-1' }), 1);
        });
    });
    // ═══════════════════════════════════════
    ...
    describe('batchUpdatePositions()', () => {
        ...
        it('should return 200 on valid batch', async () => {
            ...
            expect(response.status).toHaveBeenCalledWith(200);
            expect(response.json).toHaveBeenCalledWith(expect.objectContaining({ success: true, data: { updatedCount: 2 } }));
        });
    });
    // ═══════════════════════════════════════
    // promoteReservation
    ...
    describe('promoteReservation()', () => {
        ...
        it('should throw 401 when no user', async () => {
            await expect(controller.promoteReservation(req({ user: undefined, params: { id: 'q-1' }, body: VALID_PROMOTE }), res())).rejects.toMatchObject({ statusCode: 401 });
        });
        it('should throw 400 when hallId missing', async () => {
            const body = { ...VALID_PROMOTE, hallId: undefined };
            await expect(controller.promoteReservation(req({ params: { id: 'q-1' }, body }), res())).rejects.toMatchObject({ statusCode: 400 });
        });
        ...
        it('should return 200 on valid promotion', async () => {
            ...
            const response = res();
            await controller.promoteReservation(req({ params: { id: 'q-1' }, body: VALID_PROMOTE }), response);
            expect(response.status).toHaveBeenCalledWith(200);
    ...
    describe('getStats()', () => {
        it('should return 200 with stats', async () => {
            ...
            expect(response.status).toHaveBeenCalledWith(200);
            expect(response.json).toHaveBeenCalledWith(expect.objectContaining({ data: { totalInQueue: 15, avgWaitDays: 12 } }));
        });
    });
