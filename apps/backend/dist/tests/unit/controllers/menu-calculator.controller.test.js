...
const makeOption = (id, priceType, priceAmount) => ({
...
describe('menu-calculator.controller', () => {
    ...
    describe('calculatePrice()', () => {
        ...
        it('should return 400 when totalGuests is 0', async () => {
            const res = mockRes();
            await calculatePrice({ body: { packageId: 'p1' } }, res);
            expect(res.status).toHaveBeenCalledWith(400);
        });
        it('should return 404 when package not found', async () => {
            const res = mockRes();
            mockFindUnique.mockResolvedValue(null);
            await calculatePrice({ body: { packageId: 'p1', adults: 10 } }, res);
            expect(res.status).toHaveBeenCalledWith(404);
        });
        it('should warn when below minGuests', async () => {
            const res = mockRes();
            mockFindUnique.mockResolvedValue(PKG);
            await calculatePrice({ body: { packageId: 'pkg-1', adults: 5, children: 0, toddlers: 0 } }, res);
        ...
        it('should warn when above maxGuests', async () => {
            const res = mockRes();
            mockFindUnique.mockResolvedValue(PKG);
            await calculatePrice({ body: { packageId: 'pkg-1', adults: 250, children: 0, toddlers: 0 } }, res);
            const data = res.json.mock.calls[0][0];
        ...
        it('should calculate basic package price without options', async () => {
            ...
            await calculatePrice({ body: { packageId: 'pkg-1', adults: 50, children: 10, toddlers: 5 } }, res);
            const data = res.json.mock.calls[0][0];
        ...
        it('should return 500 on error', async () => {
            ...
            await calculatePrice({ body: { packageId: 'p1', adults: 1 } }, res);
    ...
    describe('getAvailablePackages()', () => {
        it('should return 400 when eventTypeId missing', async () => {
            const res = mockRes();
            await getAvailablePackages({ query: {} }, res);
            expect(res.status).toHaveBeenCalledWith(400);
        });
        it('should return packages without date filter', async () => {
            const res = mockRes();
            mockFindMany.mockResolvedValue([{ id: 'p1' }]);
            await getAvailablePackages({ query: { eventTypeId: 'et1' } }, res);
            const data = res.json.mock.calls[0][0];
        ...
        it('should filter by date when provided', async () => {
            const res = mockRes();
            mockFindMany.mockResolvedValue([]);
            await getAvailablePackages({ query: { eventTypeId: 'et1', date: '2026-06-15' } }, res);
            const data = res.json.mock.calls[0][0];
    ...
    describe('calculateOptionPrice()', () => {
        it('should return 400 when adults missing', async () => {
            const res = mockRes();
            await calculateOptionPrice({ params: { optionId: 'o1' }, query: {} }, res);
            expect(res.status).toHaveBeenCalledWith(400);
        });
        it('should return 404 when option not found', async () => {
            const res = mockRes();
            mockFindUnique.mockResolvedValue(null);
            await calculateOptionPrice({ params: { optionId: 'o1' }, query: { adults: '10' } }, res);
            expect(res.status).toHaveBeenCalledWith(404);
        });
...
//# sourceMappingURL=menu-calculator.controller.test.js.map