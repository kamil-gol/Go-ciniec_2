...
describe('EventTypeController branches', () => {
    ...
    describe('createEventType', () => {
        it('should throw when no userId', async () => {
            const req = { body: { name: 'Test' }, user: undefined };
        ...
        it('should create with all fields', async () => {
            eventTypeService.createEventType.mockResolvedValue({ id: '1', name: 'Wedding' });
            const req = { body: { name: 'Wedding', description: 'desc', color: '#fff', isActive: true }, user: { id: 'u1' } };
            const res = mockRes();
            await ctrl.createEventType(req, res);
            expect(res.status).toHaveBeenCalledWith(201);
        });
        it('should create with only name', async () => {
            eventTypeService.createEventType.mockResolvedValue({ id: '1', name: 'Party' });
            const req = { body: { name: 'Party' }, user: { id: 'u1' } };
            const res = mockRes();
            await ctrl.createEventType(req, res);
            expect(eventTypeService.createEventType).toHaveBeenCalledWith(expect.objectContaining({ name: 'Party' }), 'u1');
        });
    });
    describe('getEventTypes', () => {
        it('should pass activeOnly=true when isActive=true', async () => {
            eventTypeService.getEventTypes.mockResolvedValue([]);
        ...
        it('should pass activeOnly=false when isActive not set', async () => {
            eventTypeService.getEventTypes.mockResolvedValue([{ id: '1' }]);
            const req = { query: {} };
            const res = mockRes();
    ...
    describe('getEventTypeById', () => {
        it('should throw notFound when null', async () => {
            eventTypeService.getEventTypeById.mockResolvedValue(null);
        ...
        it('should return eventType when found', async () => {
            eventTypeService.getEventTypeById.mockResolvedValue({ id: '1', name: 'A' });
    ...
    describe('updateEventType', () => {
        it('should throw when no userId', async () => {
            const req = { params: { id: '1' }, body: {}, user: undefined };
            await expect(ctrl.updateEventType(req, mockRes())).rejects.toThrow();
        });
        it('should include only name when only name provided', async () => {
            eventTypeService.updateEventType.mockResolvedValue({ id: '1' });
            const req = { params: { id: '1' }, body: { name: 'New' }, user: { id: 'u1' } };
        ...
        it('should include all fields when all provided', async () => {
            ...
            await ctrl.updateEventType(req, mockRes());
            expect(eventTypeService.updateEventType).toHaveBeenCalledWith('1', { name: 'A', description: 'B', color: '#000', isActive: false }, 'u1');
        });
        it('should send empty data when no fields', async () => {
            eventTypeService.updateEventType.mockResolvedValue({ id: '1' });
            const req = { params: { id: '1' }, body: {}, user: { id: 'u1' } };
            await ctrl.updateEventType(req, mockRes());
            expect(eventTypeService.updateEventType).toHaveBeenCalledWith('1', {}, 'u1');
        });
        it('should include only description and color', async () => {
            eventTypeService.updateEventType.mockResolvedValue({ id: '1' });
            const req = { params: { id: '1' }, body: { description: 'X', color: '#fff' }, user: { id: 'u1' } };
            await ctrl.updateEventType(req, mockRes());
    ...
    describe('deleteEventType', () => {
        ...
        it('should delete successfully', async () => {
            eventTypeService.deleteEventType.mockResolvedValue(undefined);
    ...
    describe('getEventTypeStats', () => {
        it('should return stats', async () => {
            eventTypeService.getEventTypeStats.mockResolvedValue([{ type: 'A', count: 5 }]);
            const req = {};
            const res = mockRes();
    ...
    describe('getPredefinedColors', () => {
        it('should return colors', async () => {
            eventTypeService.getPredefinedColors.mockReturnValue(['#f00', '#0f0']);
            const req = {};
            const res = mockRes();
...
//# sourceMappingURL=eventType.controller.branches.test.js.map