...
describe('HallController', () => {
    ...
    describe('getHallById()', () => {
        it('should return hall', async () => {
            ...
            expect(response.json).toHaveBeenCalledWith(expect.objectContaining({ data: expect.objectContaining({ name: 'Sala A' }) }));
        });
    });
    describe('createHall()', () => {
        it('should forward 401 to next when no user', async () => {
            await hallController.createHall(req({ user: undefined, body: { name: 'S', capacity: 100 } }), res(), next);
        ...
        it('should return 201 on success', async () => {
            ...
            const response = res();
            await hallController.createHall(req({ body: { name: 'Sala B', capacity: 200 } }), response, next);
    ...
    describe('deleteHall()', () => {
        ...
        it('should return 200 on success', async () => {
            ...
            expect(response.json).toHaveBeenCalledWith(expect.objectContaining({ message: 'Hall deactivated successfully' }));
        });
    });
});
//# sourceMappingURL=hall.controller.test.js.map