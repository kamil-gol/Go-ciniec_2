...
describe('ReservationController', () => {
    describe('createReservation()', () => {
        it('should throw 401 when no user', async () => {
            await expect(controller.createReservation(req({ user: undefined }), res())).rejects.toMatchObject({ statusCode: 401 });
        });
        it('should throw 400 when required fields missing', async () => {
            await expect(controller.createReservation(req({ body: {} }), res())).rejects.toMatchObject({ statusCode: 400 });
        });
        it('should throw 400 when no date info', async () => {
            await expect(controller.createReservation(req({ body: {
    ...
    describe('getReservationById()', () => {
        it('should throw 404', async () => {
            ...
            await expect(controller.getReservationById(req({ params: { id: 'x' } }), res())).rejects.toMatchObject({ statusCode: 404 });
        });
    });
    describe('checkAvailability()', () => {
        it('should throw 400 when missing params', async () => {
            await expect(controller.checkAvailability(req({ query: {} }), res())).rejects.toMatchObject({ statusCode: 400 });
        });
        it('should return available=true', async () => {
            ...
            expect(response.json).toHaveBeenCalledWith(expect.objectContaining({ data: expect.objectContaining({ available: true }) }));
        });
    });
    describe('updateReservation()', () => {
        it('should throw 400 when reason too short for important changes', async () => {
            await expect(controller.updateReservation(req({ params: { id: 'r-1' }, body: { adults: 60, reason: 'short' } }), res())).rejects.toMatchObject({ statusCode: 400 });
        });
        it('should return 200', async () => {
    ...
    describe('updateStatus()', () => {
        it('should throw 400 when no status', async () => {
            await expect(controller.updateStatus(req({ params: { id: 'r-1' }, body: {} }), res())).rejects.toMatchObject({ statusCode: 400 });
        });
        it('should throw 400 when cancelling with paid deposits', async () => {
            ...
            await expect(controller.updateStatus(req({ params: { id: 'r-1' }, body: { status: 'CANCELLED' } }), res())).rejects.toMatchObject({ statusCode: 400 });
        });
        it('should return 200', async () => {
    ...
    describe('cancelReservation()', () => {
        it('should throw 400 when paid deposits', async () => {
            ...
            await expect(controller.cancelReservation(req({ params: { id: 'r-1' }, body: {} }), res())).rejects.toMatchObject({ statusCode: 400 });
        });
        it('should return 200', async () => {
            depSvc.checkPaidDepositsBeforeCancel.mockResolvedValue({ hasPaidDeposits: false });
