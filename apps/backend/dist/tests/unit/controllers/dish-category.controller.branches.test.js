...
const req = (overrides = {}) => ({ body: {}, params: {}, query: {}, ...overrides });
const res = () => {
    const r = {};
...
describe('DishCategoryController — branches', () => {
    ...
    describe('updateCategory()', () => {
        ...
        it('should update all fields when provided', async () => {
            ...
            const response = res();
            await dishCategoryController.updateCategory(req({ params: { id: 'c-1' }, body: { slug: 'desery', name: 'Desery', icon: '🍰', color: '#0f0', displayOrder: 5, isActive: false } }), response);
        ...
        it('should throw 409 when new slug conflicts with another category', async () => {
            ...
            await expect(dishCategoryController.updateCategory(req({ params: { id: 'c-1' }, body: { slug: 'desery' } }), res())).rejects.toMatchObject({ statusCode: 409 });
        });
        it('should skip slug conflict check when slug unchanged', async () => {
            db.findUnique.mockResolvedValue(EXISTING);
            db.update.mockResolvedValue(EXISTING);
            const response = res();
            await dishCategoryController.updateCategory(req({ params: { id: 'c-1' }, body: { slug: 'ZUPY' } }), response);
        ...
        it('should skip slug conflict check when slug not provided', async () => {
            ...
            const response = res();
            await dishCategoryController.updateCategory(req({ params: { id: 'c-1' }, body: { name: 'Updated' } }), response);
            expect(db.findFirst).not.toHaveBeenCalled();
        });
        it('should update only name when only name provided', async () => {
            db.findUnique.mockResolvedValue(EXISTING);
            db.update.mockResolvedValue({ ...EXISTING, name: 'New Name' });
            await dishCategoryController.updateCategory(req({ params: { id: 'c-1' }, body: { name: 'New Name' } }), res());
        ...
        it('should update only icon and color when provided', async () => {
            db.findUnique.mockResolvedValue(EXISTING);
            db.update.mockResolvedValue(EXISTING);
            await dishCategoryController.updateCategory(req({ params: { id: 'c-1' }, body: { icon: '🥗', color: '#00f' } }), res());
            const updateCall = db.update.mock.calls[0][0];
        ...
        it('should update displayOrder alone', async () => {
            db.findUnique.mockResolvedValue(EXISTING);
            db.update.mockResolvedValue(EXISTING);
            await dishCategoryController.updateCategory(req({ params: { id: 'c-1' }, body: { displayOrder: 10 } }), res());
            expect(db.update.mock.calls[0][0].data).toEqual({ displayOrder: 10 });
        });
        it('should update isActive alone', async () => {
            db.findUnique.mockResolvedValue(EXISTING);
            db.update.mockResolvedValue(EXISTING);
            await dishCategoryController.updateCategory(req({ params: { id: 'c-1' }, body: { isActive: false } }), res());
    ...
    describe('createCategory() — defaults', () => {
        it('should use default displayOrder=0 and isActive=true', async () => {
            ...
            const response = res();
            await dishCategoryController.createCategory(req({ body: { slug: 'test', name: 'Test' } }), response);
        ...
        it('should use provided displayOrder and isActive', async () => {
            ...
            db.create.mockResolvedValue({ id: 'c-new', slug: 'TEST', name: 'Test' });
            const response = res();
            await dishCategoryController.createCategory(req({ body: { slug: 'test', name: 'Test', displayOrder: 5, isActive: false } }), response);
            const createCall = db.create.mock.calls[0][0];
        ...
        it('should include icon and color in creation', async () => {
            ...
            await dishCategoryController.createCategory(req({ body: { slug: 'test', name: 'Test', icon: '🍕', color: '#abc' } }), res());
            const createCall = db.create.mock.calls[0][0];
...
//# sourceMappingURL=dish-category.controller.branches.test.js.map