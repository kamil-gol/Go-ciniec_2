...
const svc = companySettingsService;
const req = (overrides = {}) => ({
...
describe('CompanySettingsController', () => {
    it('getSettings — returns settings', async () => {
        ...
        expect(response.json).toHaveBeenCalledWith(expect.objectContaining({ success: true, data: expect.objectContaining({ companyName: 'Gościniec' }) }));
    });
    ...
    it('updateSettings — returns updated settings', async () => {
        ...
        expect(response.json).toHaveBeenCalledWith(expect.objectContaining({ success: true, message: expect.stringContaining('zaktualizowane') }));
    });
});
//# sourceMappingURL=company-settings.controller.test.js.map