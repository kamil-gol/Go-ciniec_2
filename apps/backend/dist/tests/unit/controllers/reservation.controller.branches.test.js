...
jest.mock('../../../utils/AppError', () => {
    class MockAppError extends Error {
        ...
        static unauthorized(msg) { return new MockAppError(msg || 'Unauthorized', 401); }
        static badRequest(msg) { return new MockAppError(msg, 400); }
        static notFound(entity) { return new MockAppError(`${entity} not found`, 404); }
    }
...
describe('ReservationController branches', () => {
    ...
    describe('createReservation', () => {
        ...
        it('should succeed without menuPackageId', async () => {
            reservationService.createReservation.mockResolvedValue({ id: 'r1' });
            const req = { body: BASE_BODY, user: { id: 'u1' } };
            const res = mockRes();
            await ctrl.createReservation(req, res);
            expect(res.json).toHaveBeenCalledWith(expect.objectContaining({
    ...
    describe('checkAvailability', () => {
        ...
        it('should throw on invalid date format', async () => {
            const req = { query: { hallId: 'h1', startDateTime: 'bad', endDateTime: 'bad' } };
    ...
    describe('getReservations', () => {
        it('should pass archived=true', async () => {
            ...
            const req = { query: { archived: 'true' } };
            await ctrl.getReservations(req, mockRes());
            expect(reservationService.getReservations).toHaveBeenCalledWith(expect.objectContaining({ archived: true }));
        });
        it('should pass archived=false', async () => {
            reservationService.getReservations.mockResolvedValue([]);
            const req = { query: { archived: 'false' } };
            await ctrl.getReservations(req, mockRes());
            expect(reservationService.getReservations).toHaveBeenCalledWith(expect.objectContaining({ archived: false }));
        });
        it('should pass archived=undefined when not set', async () => {
            reservationService.getReservations.mockResolvedValue([]);
            const req = { query: {} };
            await ctrl.getReservations(req, mockRes());
            expect(reservationService.getReservations).toHaveBeenCalledWith(expect.objectContaining({ archived: undefined }));
        });
    });
    // ===== getReservationById =====
    ...
    describe('downloadPDF', () => {
        ...
        it('should return PDF buffer', async () => {
            ...
            pdfService.generateReservationPDF.mockResolvedValue(Buffer.from('pdf'));
    ...
    describe('updateReservation', () => {
        it('should throw unauthorized when no userId', async () => {
            const req = { params: { id: 'r1' }, body: {}, user: undefined };
        ...
        it('should allow non-important changes without reason', async () => {
            reservationService.updateReservation.mockResolvedValue({ id: 'r1' });
            const req = {
                params: { id: 'r1' },
        ...
        it('should detect children as important', async () => {
            const req = { params: { id: 'r1' }, body: { children: 20 }, user: { id: 'u1' } };
            await expect(ctrl.updateReservation(req, mockRes())).rejects.toThrow('Reason');
        });
        it('should detect toddlers as important', async () => {
            const req = { params: { id: 'r1' }, body: { toddlers: 5 }, user: { id: 'u1' } };
            await expect(ctrl.updateReservation(req, mockRes())).rejects.toThrow('Reason');
        });
        it('should detect pricePerChild as important', async () => {
            const req = { params: { id: 'r1' }, body: { pricePerChild: 100 }, user: { id: 'u1' } };
            await expect(ctrl.updateReservation(req, mockRes())).rejects.toThrow('Reason');
        });
        it('should detect pricePerToddler as important', async () => {
            const req = { params: { id: 'r1' }, body: { pricePerToddler: 50 }, user: { id: 'u1' } };
            await expect(ctrl.updateReservation(req, mockRes())).rejects.toThrow('Reason');
        });
    });
    // ===== updateStatus =====
    describe('updateStatus', () => {
        ...
        it('should block cancel with paid deposits', async () => {
            depositService.checkPaidDepositsBeforeCancel.mockResolvedValue({
                hasPaidDeposits: true, paidCount: 2, paidTotal: 1000,
            });
            const req = { params: { id: 'r1' }, body: { status: 'CANCELLED' }, user: { id: 'u1' } };
        ...
        it('should allow cancel without paid deposits', async () => {
            ...
            reservationService.updateStatus.mockResolvedValue({ id: 'r1' });
            const req = { params: { id: 'r1' }, body: { status: 'CANCELLED' }, user: { id: 'u1' } };
            const res = mockRes();
        ...
        it('should not check deposits for non-cancel status', async () => {
            reservationService.updateStatus.mockResolvedValue({ id: 'r1' });
            const req = { params: { id: 'r1' }, body: { status: 'CONFIRMED' }, user: { id: 'u1' } };
            const res = mockRes();
            await ctrl.updateStatus(req, res);
    ...
    describe('cancelReservation', () => {
        ...
        it('should block when paid deposits exist', async () => {
            ...
            const req = { params: { id: 'r1' }, body: { reason: 'test' }, user: { id: 'u1' } };
        ...
        it('should cancel successfully', async () => {
            ...
            reservationService.cancelReservation.mockResolvedValue(undefined);
            const req = { params: { id: 'r1' }, body: { reason: 'test' }, user: { id: 'u1' } };
            const res = mockRes();
    ...
    describe('archiveReservation', () => {
        ...
        it('should archive successfully', async () => {
            reservationService.archiveReservation.mockResolvedValue(undefined);
            const req = { params: { id: 'r1' }, body: { reason: 'done' }, user: { id: 'u1' } };
            const res = mockRes();
    ...
    describe('unarchiveReservation', () => {
        ...
        it('should unarchive successfully', async () => {
            reservationService.unarchiveReservation.mockResolvedValue(undefined);
            const req = { params: { id: 'r1' }, body: { reason: 'reopen' }, user: { id: 'u1' } };
            const res = mockRes();
...
//# sourceMappingURL=reservation.controller.branches.test.js.map