...
describe('EventTypeController', () => {
    describe('createEventType()', () => {
        it('should throw 401 when no user', async () => {
            await expect(controller.createEventType(req({ user: undefined, body: { name: 'Wesele' } }), res())).rejects.toMatchObject({ statusCode: 401 });
        });
        it('should throw 400 when name missing', async () => {
            await expect(controller.createEventType(req({ body: {} }), res())).rejects.toMatchObject({ statusCode: 400 });
        });
    ...
    describe('updateEventType()', () => {
        it('should throw 401 when no user', async () => {
            await expect(controller.updateEventType(req({ user: undefined, params: { id: 'et-1' } }), res())).rejects.toMatchObject({ statusCode: 401 });
        });
    ...
    describe('deleteEventType()', () => {
        it('should throw 401 when no user', async () => {
            await expect(controller.deleteEventType(req({ user: undefined, params: { id: 'et-1' } }), res())).rejects.toMatchObject({ statusCode: 401 });
        });
        it('should return 200 on success', async () => {
    ...
    it('getPredefinedColors — returns colors', async () => {
        ...
        expect(response.json).toHaveBeenCalledWith(expect.objectContaining({ data: ['#FF0000', '#00FF00'] }));
    });
});
//# sourceMappingURL=eventType.controller.test.js.map