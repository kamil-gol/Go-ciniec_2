...
jest.mock('../../../validation/menu.validation', () => ({
    createMenuTemplateSchema: { parse: jest.fn((d) => d) },
    updateMenuTemplateSchema: { parse: jest.fn((d) => d) },
    duplicateMenuTemplateSchema: { parse: jest.fn((d) => d) },
    menuTemplateQuerySchema: { parse: jest.fn((d) => d) },
}));
...
const db = prisma.packageCategorySettings;
const req = (overrides = {}) => ({
...
const next = jest.fn();
function zodError() {
    return new z.ZodError([{ code: 'custom', message: 'bad', path: ['x'] }]);
}
beforeEach(() => {
    jest.clearAllMocks();
...
describe('MenuTemplateController', () => {
    ...
    describe('list()', () => {
        it('should return 200 with templates', async () => {
            ...
            expect(response.json).toHaveBeenCalledWith(expect.objectContaining({ success: true, count: 1 }));
        });
        it('should return 400 on ZodError', async () => {
            schemas.menuTemplateQuerySchema.parse.mockImplementationOnce(() => { throw zodError(); });
            const response = res();
            await ctrl.list(req(), response, next);
            expect(response.status).toHaveBeenCalledWith(400);
            expect(response.json).toHaveBeenCalledWith(expect.objectContaining({ error: 'Validation error' }));
        });
        it('should call next on unknown error', async () => {
            schemas.menuTemplateQuerySchema.parse.mockImplementationOnce(() => { throw new TypeError('fail'); });
    ...
    describe('getActive()', () => {
        it('should return 200 with date from query', async () => {
            ...
            expect(response.status).toHaveBeenCalledWith(200);
            expect(svc.getActiveMenuForEventType).toHaveBeenCalledWith('et-1', new Date('2026-06-15'));
        });
        it('should use current date when no date query', async () => {
            ...
            const callDate = svc.getActiveMenuForEventType.mock.calls[0][1];
    ...
    describe('create()', () => {
        ...
        it('should return 400 on ZodError', async () => {
            schemas.createMenuTemplateSchema.parse.mockImplementationOnce(() => { throw zodError(); });
            const response = res();
    ...
    describe('update()', () => {
        ...
        it('should return 400 on ZodError', async () => {
            schemas.updateMenuTemplateSchema.parse.mockImplementationOnce(() => { throw zodError(); });
            const response = res();
    ...
    describe('duplicate()', () => {
        ...
        it('should return 400 on ZodError', async () => {
            schemas.duplicateMenuTemplateSchema.parse.mockImplementationOnce(() => { throw zodError(); });
            const response = res();
    ...
    describe('downloadPdf()', () => {
        ...
        it('should generate and send PDF', async () => {
            ...
            expect(pdfSvc.generateMenuCardPDF).toHaveBeenCalledWith(expect.objectContaining({
        ...
        it('should return 500 on unknown error', async () => {
            ...
            expect(response.json).toHaveBeenCalledWith(expect.objectContaining({ error: 'PDF generation failed', details: 'something broke' }));
        });
        it('should return 500 with Unknown error for non-Error objects', async () => {
            ...
            await ctrl.downloadPdf(req({ params: { id: 't-1' } }), response);
            expect(response.status).toHaveBeenCalledWith(500);
            expect(response.json).toHaveBeenCalledWith(expect.objectContaining({ details: 'Unknown error' }));
        });
    });
});
//# sourceMappingURL=menuTemplate.controller.test.js.map