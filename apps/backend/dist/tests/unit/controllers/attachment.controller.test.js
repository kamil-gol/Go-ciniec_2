...
describe('AttachmentController', () => {
    describe('upload()', () => {
        ...
        it('should return 401 when no user', async () => {
            const response = res();
            await attachmentController.upload(req({ file: { originalname: 'test.pdf' }, user: undefined }), response, next);
            expect(response.status).toHaveBeenCalledWith(401);
        });
        it('should return 201 on success', async () => {
            svc.createAttachment.mockResolvedValue({ id: 'a-1' });
            const response = res();
            await attachmentController.upload(req({
    ...
    describe('getByEntity()', () => {
        ...
        it('should return attachments', async () => {
            ...
            const response = res();
            await attachmentController.getByEntity(req({ query: { entityType: 'RESERVATION', entityId: 'r-1' } }), response, next);
        ...
        it('should use withClientRodo method', async () => {
            ...
            const response = res();
            await attachmentController.getByEntity(req({ query: { entityType: 'RESERVATION', entityId: 'r-1', withClientRodo: 'true' } }), response, next);
    ...
    describe('update()', () => {
        it('should return updated attachment', async () => {
            ...
            const response = res();
            await attachmentController.update(req({ params: { id: 'a-1' }, body: { label: 'Updated' } }), response, next);
    ...
    describe('check()', () => {
        ...
        it('should return has=true', async () => {
            ...
            const response = res();
            await attachmentController.check(req({ query: { entityType: 'CLIENT', entityId: 'c-1', category: 'RODO' } }), response, next);
            expect(response.json).toHaveBeenCalledWith(expect.objectContaining({ data: { has: true } }));
        });
    });
    describe('batchCheckRodo()', () => {
        ...
        it('should return result', async () => {
            ...
            const response = res();
            await attachmentController.batchCheckRodo(req({ body: { clientIds: ['c-1'] } }), response, next);
    ...
    describe('batchCheckContract()', () => {
        ...
        it('should return result', async () => {
            ...
            const response = res();
            await attachmentController.batchCheckContract(req({ body: { reservationIds: ['r-1'] } }), response, next);
            expect(response.json).toHaveBeenCalledWith(expect.objectContaining({ data: { 'r-1': false } }));
        });
    });
});
//# sourceMappingURL=attachment.controller.test.js.map