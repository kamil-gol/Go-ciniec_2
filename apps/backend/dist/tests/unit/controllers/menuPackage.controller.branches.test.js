...
jest.mock('../../../validation/menu.validation', () => ({
    createMenuPackageSchema: { parse: jest.fn((d) => d) },
    updateMenuPackageSchema: { parse: jest.fn((d) => d) },
    reorderPackagesSchema: { parse: jest.fn((d) => d) },
    assignOptionsToPackageSchema: { parse: jest.fn((d) => d) },
}));
...
const schemas = {
    create: createMenuPackageSchema,
    update: updateMenuPackageSchema,
    reorder: reorderPackagesSchema,
    assign: assignOptionsToPackageSchema,
};
const req = (overrides = {}) => ({
    body: {}, params: {}, query: {},
    user: { id: 'user-001' },
...
beforeEach(() => {
    jest.clearAllMocks();
    schemas.create.parse.mockImplementation((d) => d);
    schemas.update.parse.mockImplementation((d) => d);
    schemas.reorder.parse.mockImplementation((d) => d);
    schemas.assign.parse.mockImplementation((d) => d);
});
...
//# sourceMappingURL=menuPackage.controller.branches.test.js.map