...
describe('Queue API — /api/queue', () => {
    ...
    function futureDate(monthsAhead = 3) {
        const d = new Date();
        d.setMonth(d.getMonth() + monthsAhead);
        return d;
    }
    function futureDateStr(monthsAhead = 3) {
     ...
     */
    async function createQueueItemInDb(overrides = {}) {
     ...
     */
    async function createMultipleQueueItems(count) {
        const items = [];
    ...
    describe('POST /api/queue/batch-update-positions', () => {
        ...
        it('[BUG9b] should handle two concurrent batch updates without corruption', async () => {
            ...
            if (queueRes.body.data && queueRes.body.data.length > 0) {
                const positions = queueRes.body.data.map((item) => item.position || item.reservationQueuePosition).filter((p) => p != null);
...
//# sourceMappingURL=queue.api.test.js.map