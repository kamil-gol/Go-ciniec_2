...
describe('Deposit API', () => {
    ...
    // ── API helpers ──
    async function createDeposit(resId, amount, dueDate = '2026-06-01') {
        return api
            .post(`/api/reservations/${resId}/deposits`)
            .set(adminAuth())
            .send({ amount, dueDate });
    }
    async function markPaid(depositId, paymentMethod = 'TRANSFER', paidAt = '2026-02-20') {
        return api
    ...
    describe('GET /api/deposits', () => {
        ...
        it('should support sorting by amount desc', async () => {
            ...
            const res = await api
                ...
                .set(adminAuth());
            expect(res.status).toBe(200);
            const amounts = res.body.data.map((d) => Number(d.amount));
    ...
    describe('Authorization — all endpoints require auth', () => {
        it('should return 401 for all endpoints without token', async () => {
            ...
            const endpoints = [
                { method: 'get', url: '/api/deposits' },
                { method: 'get', url: '/api/deposits/stats' },
                { method: 'get', url: '/api/deposits/overdue' },
                { method: 'get', url: `/api/deposits/${depositId}` },
                { method: 'put', url: `/api/deposits/${depositId}` },
                { method: 'delete', url: `/api/deposits/${depositId}` },
                { method: 'patch', url: `/api/deposits/${depositId}/mark-paid` },
                { method: 'patch', url: `/api/deposits/${depositId}/mark-unpaid` },
                { method: 'patch', url: `/api/deposits/${depositId}/cancel` },
                { method: 'get', url: `/api/reservations/${reservationId}/deposits` },
                { method: 'post', url: `/api/reservations/${reservationId}/deposits` },
            ];
...
//# sourceMappingURL=deposit.api.test.js.map