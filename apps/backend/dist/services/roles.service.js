...
class RolesService {
    ...
    async createRole(data, actorId) {
        ...
        if (existing)
            throw AppError.conflict('Rola z tym slugiem już istnieje');
        const nameExists = await prisma.role.findUnique({ where: { name: data.name } });
        if (nameExists)
            throw AppError.conflict('Rola z tą nazwą już istnieje');
    ...
    async updateRolePermissions(id, permissionIds, actorId) {
        ...
        await prisma.$transaction([
            ...
            ...permissionIds.map((permId) => prisma.rolePermission.create({
    ...
    async deleteRole(id, actorId) {
        ...
        if (role._count.users > 0) {
            throw AppError.badRequest(`Nie można usunąć roli przypisanej do ${role._count.users} użytkowników. ` +
