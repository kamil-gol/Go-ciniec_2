...
export class MenuSnapshotService {
    ...
    calculatePriceBreakdown(menuData, adultsCount, childrenCount, toddlersCount) {
    ...
    async getSnapshotByReservationId(reservationId) {
    ...
    async deleteSnapshot(reservationId) {
        return prisma.reservationMenuSnapshot.delete({ where: { reservationId } });
    }
    async hasSnapshot(reservationId) {
    ...
    async getPopularOptions(limit = 10) {
        ...
        snapshots.forEach(snapshot => {
            ...
            menuData.selectedOptions.forEach(opt => {
                ...
                if (!optionCounts[opt.optionId])
                    optionCounts[opt.optionId] = { name: opt.name, count: 0 };
    ...
    async getPopularPackages(limit = 10) {
        ...
        snapshots.forEach(snapshot => {
            ...
            if (!packageCounts[pkgId])
                packageCounts[pkgId] = { name: menuData.packageName, count: 0 };
...
//# sourceMappingURL=menuSnapshot.service.js.map