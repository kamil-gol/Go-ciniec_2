...
class UsersService {
    async getUsers(params) {
        const { page = 1, limit = 20, search, roleId, isActive, sortBy = 'createdAt', sortOrder = 'desc', } = params;
        const where = {};
        if (search) {
            where.OR = [
    ...
    async createUser(data, actorId) {
        ...
        if (existing)
            throw AppError.conflict('Użytkownik z tym adresem email już istnieje');
    ...
    async updateUser(id, data, actorId) {
        ...
        if (data.email && data.email !== existing.email) {
            ...
            if (emailTaken)
                throw AppError.conflict('Ten adres email jest już zajęty');
        }
        if (data.roleId) {
            const role = await prisma.role.findUnique({ where: { id: data.roleId } });
            if (!role)
    ...
    async changePassword(id, newPassword, actorId) {
