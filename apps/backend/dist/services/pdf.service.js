...
export class PDFService {
    ...
    async generateReservationPDF(reservation) {
    ...
    async generatePaymentConfirmationPDF(data) {
        return new Promise((resolve, reject) => {
    ...
    // ═══════════════ MENU CARD PDF ═══════════════
    async generateMenuCardPDF(data) {
        return new Promise((resolve, reject) => {
    ...
    buildPDFContent(doc, reservation) {
        ...
        if (reservation.adults > 0 && reservation.pricePerAdult > 0) {
            ...
            doc.text(`Dorosli: ${reservation.adults} os. x ${this.formatCurrency(reservation.pricePerAdult)} = ${this.formatCurrency(adultTotal)}`);
        }
        ...
        if (reservation.children > 0 && reservation.pricePerChild > 0) {
            ...
            doc.text(`Dzieci (4-12 lat): ${reservation.children} os. x ${this.formatCurrency(reservation.pricePerChild)} = ${this.formatCurrency(childTotal)}`);
        }
        ...
        if (reservation.toddlers > 0 && reservation.pricePerToddler > 0) {
            ...
            doc.text(`Maluchy (0-3 lata): ${reservation.toddlers} os. x ${this.formatCurrency(reservation.pricePerToddler)} = ${this.formatCurrency(toddlerTotal)}`);
        }
        doc.moveDown(0.5);
    ...
    buildPaymentConfirmationContent(doc, data) {
        this.addHeader(doc);
        doc.moveDown(2);
    ...
    buildMenuCardContent(doc, data) {
        ...
        const subtitleParts = [data.eventTypeName];
        if (data.variant)
            subtitleParts.push(data.variant);
        doc.moveDown(0.3);
        ...
        doc.moveDown(0.5);
        doc.fontSize(8).fillColor('#999999').text(`Wygenerowano: ${this.formatDate(new Date())}`, { align: 'center' });
        data.packages.forEach((pkg) => {
            ...
            if (pkg.pricePerChild > 0)
                priceParts.push(`${this.formatCurrency(pkg.pricePerChild)}/dziecko`);
            if (pkg.pricePerToddler > 0)
                priceParts.push(`${this.formatCurrency(pkg.pricePerToddler)}/maluch`);
    ...
    // ═══════════════ SHARED HELPERS ═══════════════
    addMenuSelectionSection(doc, menuSnapshot, pageWidth) {
        doc.moveDown(1);
        this.addSeparator(doc);
        doc.moveDown(1);
        ...
        if (dishSelections.length === 0) {
            ...
            dishSelections.forEach((category, categoryIndex) => {
                doc.fontSize(12).font(this.getBoldFont()).fillColor('#2c3e50');
    ...
    addMenuSelectionSectionLegacy(doc, menuData, pageWidth) {
        doc.moveDown(1);
        this.addSeparator(doc);
        doc.moveDown(1);
    ...
    addFooter(doc) {
        ...
        doc.text('Dziekujemy za wybranie naszej restauracji. W razie pytan prosimy o kontakt.', 50, bottomY, {
            align: 'center',
            width: doc.page.width - 100,
        });
        doc.moveDown(0.5);
        doc.fontSize(7).text(`Dokument wygenerowany automatycznie przez system ${this.restaurantData.name}`, {
            align: 'center',
        });
    }
    getRegularFont() {
