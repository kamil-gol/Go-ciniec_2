...
import { ENTITY_TYPES, isValidCategory, STORAGE_DIRS } from '../constants/attachmentCategories';
...
class AttachmentService {
     ...
     */
    moveToEntityDir(file, entityType) {
     ...
     */
    async resolveClientId(entityType, entityId) {
        try {
     ...
     */
    async createAttachment(dto, file, uploadedById) {
        // Validate entityType
        ...
        // ═══════════════════════════════════════════════════════════
        let finalEntityType = dto.entityType;
        let finalEntityId = dto.entityId;
        if (dto.category === 'RODO' && dto.entityType !== 'CLIENT') {
            const clientId = await this.resolveClientId(dto.entityType, dto.entityId);
            if (!clientId) {
                throw AppError.badRequest(`Nie udało się znaleźć klienta powiązanego z ${dto.entityType} ${dto.entityId}. ` +
        ...
        logger.info(`Attachment created: ${attachment.id} (${finalEntityType}/${finalEntityId}, ` +
     ...
     */
    async getAttachmentsWithClientRodo(entityType, entityId, includeArchived = false) {
        // Get own attachments
     ...
     */
    async updateAttachment(id, dto, userId) {
        const existing = await this.getAttachmentById(id);
        // If changing category, validate it
        if (dto.category && !isValidCategory(existing.entityType, dto.category)) {
     ...
     */
    async hardDeleteAttachment(id, userId) {
        const attachment = await this.getAttachmentById(id);
     ...
     */
    async countByCategory(entityType, entityId) {
     ...
     */
    async hasAttachment(entityType, entityId, category) {
     ...
     */
    async batchCheckRodo(clientIds) {
     ...
     */
    async batchCheckContract(reservationIds) {
