...
const depositService = {
    async create(input, userId) {
        ...
        if (existingDepositsSum + amount > totalPrice) {
            throw AppError.badRequest('Suma zaliczek (' + (existingDepositsSum + amount) + ' PLN) przekracza cene rezerwacji (' + totalPrice + ' PLN). ' +
        ...
        const result = await prisma.$queryRawUnsafe(`INSERT INTO "Deposit" (
      ...
      ) RETURNING id::text as id`, reservationId, amount, amount, dueDateStr);
    ...
    async list(filters) {
        const { reservationId, status, overdue, dateFrom, dateTo, paid, search, page = 1, limit = 20, sortBy = 'dueDate', sortOrder = 'asc', } = filters;
        const where = {};
        if (reservationId)
        ...
        if (dateFrom || dateTo) {
            const dueDateFilter = {};
            if (dateFrom)
                dueDateFilter.gte = dateFrom.substring(0, 10);
            if (dateTo)
                dueDateFilter.lte = dateTo.substring(0, 10);
        ...
        if (input.amount !== undefined) {
            ...
            if (reservation) {
                ...
                if (otherDepositsSum + input.amount > totalPrice) {
                    throw AppError.badRequest('Suma zaliczek (' + (otherDepositsSum + input.amount) + ' PLN) przekracza cene rezerwacji (' + totalPrice + ' PLN).');
                }
            }
        }
        if (input.amount !== undefined && input.dueDate) {
            const dueDateStr = input.dueDate.substring(0, 10);
            await prisma.$queryRawUnsafe(`UPDATE "Deposit" SET amount = $1, "remainingAmount" = $2, "dueDate" = $3, "updatedAt" = NOW() WHERE id = $4::uuid`, input.amount, input.amount, dueDateStr, id);
        }
        else if (input.amount !== undefined) {
            await prisma.$queryRawUnsafe(`UPDATE "Deposit" SET amount = $1, "remainingAmount" = $2, "updatedAt" = NOW() WHERE id = $3::uuid`, input.amount, input.amount, id);
        }
        else if (input.dueDate) {
            const dueDateStr = input.dueDate.substring(0, 10);
            await prisma.$queryRawUnsafe(`UPDATE "Deposit" SET "dueDate" = $1, "updatedAt" = NOW() WHERE id = $2::uuid`, dueDateStr, id);
        }
    ...
    async markAsPaid(id, input, userId) {
        const deposit = await prisma.deposit.findUnique({ where: { id } });
        if (!deposit)
            throw AppError.notFound('Deposit');
        if (deposit.paid)
        ...
        await prisma.$queryRawUnsafe(`UPDATE "Deposit" SET paid = $1, status = $2, "paidAt" = $3::timestamp, "paymentMethod" = $4, "remainingAmount" = $5, "paidAmount" = $6, "updatedAt" = NOW() WHERE id = $7::uuid`, isPaid, newStatus, input.paidAt, input.paymentMethod, remainingAmount, amountPaid, id);
        const updated = await prisma.deposit.findUnique({
            where: { id },
            include: DEPOSIT_INCLUDE,
        });
     ...
     */
    async checkAndAutoConfirmReservation(reservationId, userId) {
        try {
            const reservation = await prisma.reservation.findUnique({
                where: { id: reservationId },
                include: { deposits: true, client: true },
            ...
            if (reservation.status !== 'PENDING')
                return;
            const activeDeposits = reservation.deposits.filter((d) => d.status !== 'CANCELLED');
            // Need at least one active deposit
            if (activeDeposits.length === 0)
                return;
            const allPaid = activeDeposits.every((d) => d.status === 'PAID');
            if (allPaid) {
                ...
                await logChange({
                    ...
                    details: {
                        ...
                        totalPaid: activeDeposits.reduce((s, d) => s + Number(d.amount), 0),
                    },
                });
     ...
     */
    async checkPaidDepositsBeforeCancel(reservationId) {
        const deposits = await prisma.deposit.findMany({
            where: { reservationId, status: 'PAID' },
        });
        const paidCount = deposits.length;
        const paidTotal = deposits.reduce((sum, d) => sum + Number(d.amount), 0);
    ...
    async sendConfirmationEmail(id) {
        ...
        await emailService.sendDepositPaidConfirmation(client.email, {
    ...
    async markAsUnpaid(id, userId) {
        ...
        await prisma.$queryRawUnsafe(`UPDATE "Deposit" SET paid = false, status = 'PENDING', "paidAt" = NULL, "paymentMethod" = NULL, "remainingAmount" = $1, "paidAmount" = 0, "updatedAt" = NOW() WHERE id = $2::uuid`, depositAmount, id);
        const updated = await prisma.deposit.findUnique({
            where: { id },
            include: DEPOSIT_INCLUDE,
        });
    ...
    async cancel(id, userId) {
        ...
        await prisma.$queryRawUnsafe(`UPDATE "Deposit" SET status = 'CANCELLED', "remainingAmount" = 0, "updatedAt" = NOW() WHERE id = $1::uuid`, id);
        const updated = await prisma.deposit.findUnique({
            where: { id },
            include: DEPOSIT_INCLUDE,
        });
    ...
    async autoMarkOverdue() {
        const todayStr = new Date().toISOString().substring(0, 10);
        const result = await prisma.$queryRawUnsafe(`WITH updated AS (
        UPDATE "Deposit" SET status = 'OVERDUE', "updatedAt" = NOW()
        WHERE status = 'PENDING' AND paid = false AND "dueDate" < $1
        RETURNING id
      ) SELECT count(*)::int as count FROM updated`, todayStr);
