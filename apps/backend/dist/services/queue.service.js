...
const withRetry = async (operation, maxRetries = 3, baseDelay = 100) => {
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
        try {
            ...
            lastError = error;
            const isLockError = error instanceof Prisma.PrismaClientKnownRequestError && error.code === 'P2034' ||
...
export class QueueService {
    ...
    async updateQueueReservation(reservationId, data, userId) {
    ...
    async swapPositions(id1, id2, userId) {
        if (!id1 || !id2)
            throw AppError.badRequest('Both reservation IDs are required');
        if (id1 === id2)
            throw AppError.badRequest('Cannot swap reservation with itself');
        ...
        const client1Name = res1.client
            ? `${res1.client.firstName} ${res1.client.lastName}`
            : 'N/A';
        const client2Name = res2.client
            ? `${res2.client.firstName} ${res2.client.lastName}`
            : 'N/A';
    ...
    async moveToPosition(reservationId, newPosition, userId) {
        if (!reservationId)
    ...
    async batchUpdatePositions(updates, userId) {
    ...
    async promoteReservation(reservationId, data, userId) {
        const reservation = await prisma.reservation.findUnique({
            where: { id: reservationId },
            include: { client: true },
        });
     ...
     */
    async autoCancelExpired(userId) {
