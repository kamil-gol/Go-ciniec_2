...
export class DiscountService {
     ...
     */
    async applyDiscount(id, data, userId) {
        const user = await prisma.user.findUnique({ where: { id: userId } });
        if (!user)
            throw new AppError(401, 'Sesja wygasła lub użytkownik nie istnieje');
        ...
        if (data.type === 'PERCENTAGE') {
            if (data.value > 100)
                throw new AppError(400, 'Rabat procentowy nie może przekroczyć 100%');
     ...
     */
    async removeDiscount(id, userId) {
        const user = await prisma.user.findUnique({ where: { id: userId } });
        if (!user)
            throw new AppError(401, 'Sesja wygasła lub użytkownik nie istnieje');
        const reservation = await prisma.reservation.findUnique({
