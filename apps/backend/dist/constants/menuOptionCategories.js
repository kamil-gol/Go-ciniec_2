 ...
 * ```
 */
export function isValidMenuOptionCategory(category) {
    return VALID_MENU_OPTION_CATEGORIES.includes(category);
}
/**
 ...
 * ```
 */
export function normalizeCategoryFormat(category) {
    // Check if already in standard format
    if (isValidMenuOptionCategory(category)) {
        return category;
    }
    // Try legacy format conversion
    return LEGACY_CATEGORY_FORMATS[category] || category;
}
/**
 ...
 * ```
 */
export function getAllCategoriesWithTranslations() {
    return VALID_MENU_OPTION_CATEGORIES.map(english => ({
        english,
        polish: CATEGORY_TRANSLATIONS[english],
    }));
}
//# sourceMappingURL=menuOptionCategories.js.map