...
export async function loadUserPermissions(userId) {
    ...
    if (!user || !user.assignedRole) {
        ...
        const legacyPerms = getLegacyPermissions(user?.legacyRole);
        const result = {
            permissions: new Set(legacyPerms),
            roleName: user?.legacyRole || 'UNKNOWN',
            roleSlug: (user?.legacyRole || 'unknown').toLowerCase(),
            cachedAt: Date.now(),
        };
        permissionCache.set(userId, result);
        return result;
    }
    const permissions = new Set(user.assignedRole.permissions.map((rp) => rp.permission.slug));
    const result = {
        permissions,
 ...
 */
function getLegacyPermissions(legacyRole) {
    switch (legacyRole) {
        case 'ADMIN':
...
export const requirePermission = (...requiredPermissions) => {
    return async (req, _res, next) => {
        try {
            ...
            if (missing.length > 0) {
                logger.warn(`Brak uprawnień: ${req.user.email} (${roleName}) ` +
                    `próbował wykonać akcję wymagającą: ${missing.join(', ')}`);
                return next(new AppError(403, `Brak wymaganych uprawnień: ${missing.join(', ')}`));
            }
    ...
    return async (req, _res, next) => {
        try {
            ...
            if (!hasAny) {
                ...
                return next(new AppError(403, 'Brak wymaganych uprawnień'));
            }
            req.userPermissions = permissions;
            req.userRoleName = roleName;
            next();
 ...
 */
export const attachPermissionCheck = (...permissionsToCheck) => {
    return async (req, _res, next) => {
        try {
            if (!req.user) {
