...
import { createMenuTemplateSchema, updateMenuTemplateSchema, duplicateMenuTemplateSchema, menuTemplateQuerySchema } from '../validation/menu.validation';
import { z } from 'zod';
export class MenuTemplateController {
    ...
    async getActive(req, res, next) {
        try {
            ...
            const template = await menuService.getActiveMenuForEventType(eventTypeId, menuDate);
            return res.status(200).json({
                success: true,
                data: template
            });
    ...
    async downloadPdf(req, res) {
        try {
            ...
            const packagesWithCourses = await Promise.all((template.packages || []).map(async (pkg) => {
...
//# sourceMappingURL=menuTemplate.controller.js.map