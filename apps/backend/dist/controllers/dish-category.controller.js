...
class DishCategoryController {
    ...
    async getCategoryBySlug(req, res) {
        ...
        if (!category)
            throw AppError.notFound(`Category with slug "${slug}"`);
        res.status(200).json({
            success: true,
            data: category,
        });
    ...
    async updateCategory(req, res) {
        ...
        const updateData = {};
        if (slug !== undefined)
            updateData.slug = slug.toUpperCase();
        if (name !== undefined)
    ...
    async deleteCategory(req, res) {
        ...
        if (category._count.dishes > 0) {
            throw AppError.conflict(`Cannot delete category "${category.name}" because it has ${category._count.dishes} dish(es) assigned`);
        }
        await prisma.$transaction(async (tx) => {
            ...
            const reorderUpdates = remainingCategories.map((cat, index) => tx.dishCategory.update({
                where: { id: cat.id },
    ...
    async reorderCategories(req, res) {
        ...
        const updates = ids.map((id, index) => prisma.dishCategory.update({
            where: { id },
            data: { displayOrder: index },
        }));
...
//# sourceMappingURL=dish-category.controller.js.map